  ...
  <div class="md:col-span-1 space-y-4">
    ...
    <div class="bg-white border border-gray-100 rounded-2xl p-5 space-y-3">
      <div class="flex items-center gap-2 text-sm text-gray-700">
        ...
        <span>{{ \App\Models\AppSetting::get('support_email', 'support@example.com') }}</span>
      </div>
    ...
    @auth
    <a href="{{ route('support.index') }}" class="block bg-white border border-gray-100 rounded-2xl p-4 hover:border-indigo-200 transition">
    ...
    <div class="bg-white border border-gray-100 rounded-2xl p-5">
      ...
      <div class="space-y-2 text-xs">
        ...
        <div class="flex items-center justify-between">
          <span class="px-2 py-0.5 bg-{{ $p['color'] }}-100 text-{{ $p['color'] }}-700 rounded font-medium">{{ $p['label'] }}</span>
          <span class="text-gray-600">{{ $p['sla_hours'] }} ชั่วโมง</span>
        </div>
        @endforeach
      </div>
    </div>
      ...
      <form method="POST" action="{{ route('contact.store') }}" class="space-y-4">
        ...
        <div>
          ...
          <textarea name="message" required rows="6" maxlength="5000"
                    ...
                    class="w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-indigo-200 focus:border-indigo-400">{{ old('message') }}</textarea>
