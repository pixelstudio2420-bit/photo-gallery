...
<head>
  ...
  <title>@yield('title', 'Dashboard') — Admin | {{ $siteName ?? config('app.name') }}</title>
...
<div class="min-h-screen">
  ...
  <div class="transition-all duration-300 min-h-screen flex flex-col bg-gray-50 dark:bg-[#0f1419]"
    ...
    <header class="sticky top-0 z-[1030] h-16 bg-white/80 backdrop-blur-xl border-b border-gray-200/60
      ...
      {{-- Sidebar toggle — tooltip shows current state so users discover the action --}}
      <button type="button"
        ...
        {{-- Pulse hint when collapsed (mobile + desktop) --}}
      ...
      <div class="ml-auto flex items-center gap-2">
        ...
        {{-- Language switcher (only shown when multi-lang is enabled AND more than 1 enabled locale) --}}
        @php
          ...
          $_adminMeta = $_adminEnabledLocales[$_adminCurrent]
              ...
              ?? ['flag' => '🇹🇭', 'native' => 'ไทย'];
        @endphp
        @if($_adminMultilangOn && count($_adminEnabledLocales) > 1)
        <div x-data="{ open: false }" class="relative">
          <button @click="open = !open" @click.outside="open = false" type="button"
                  class="w-9 h-9 rounded-xl bg-gray-100 text-gray-600 flex items-center justify-center
                         hover:bg-gray-200 transition-colors dark:bg-white/[0.06] dark:text-gray-300 dark:hover:bg-white/10"
                  title="Language">
            <span class="text-base leading-none">{{ $_adminMeta['flag'] }}</span>
          </button>
          <div x-show="open" x-cloak
            ...
            @foreach($_adminEnabledLocales as $code => $meta)
              <a href="{{ route('lang.switch', $code) }}?redirect={{ urlencode(request()->getRequestUri()) }}"
                 class="flex items-center gap-2.5 px-4 py-2.5 text-sm hover:bg-indigo-50 dark:hover:bg-indigo-500/10 transition {{ $_adminCurrent === $code ? 'bg-indigo-50 dark:bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 font-semibold' : 'text-gray-700 dark:text-gray-300' }}">
                <span class="text-lg leading-none">{{ $meta['flag'] }}</span>
                <span class="flex-1">{{ $meta['native'] }}</span>
                @if($_adminCurrent === $code)<i class="bi bi-check-circle-fill text-indigo-600"></i>@endif
              </a>
            @endforeach
          </div>
        </div>
        ...
        {{-- Reset UI Preferences — clears sidebar/theme localStorage so a new
             admin user (or one who got into a confusing state) gets a fresh
             default layout. Confirms before reload to avoid accidental clicks. --}}
        ...
        <div class="hidden sm:flex items-center gap-2.5 pl-2 border-l border-gray-200 dark:border-white/10 ml-1">
          @php $_adm = Auth::guard('admin')->user(); $_ri = $_adm->role_info; @endphp
          <div class="w-8 h-8 rounded-lg text-xs font-bold flex items-center justify-center"
               style="background:linear-gradient(135deg,{{ $_ri['color'] }}30,{{ $_ri['color'] }}10);color:{{ $_ri['color'] }};">
            {{ mb_strtoupper(mb_substr($_adm->full_name ?? 'A', 0, 1, 'UTF-8'), 'UTF-8') }}
          </div>
          <div class="flex flex-col leading-tight">
            <span class="text-sm font-medium text-slate-700 dark:text-gray-200">{{ $_adm->full_name ?? 'Admin' }}</span>
            <span class="text-[0.6rem] font-semibold" style="color:{{ $_ri['color'] }};">{{ $_ri['thai'] }}</span>
          </div>
        </div>
...
<script src="{{ asset('js/admin-realtime-search.js') }}"></script>
...
@stack('scripts')
<script src="{{ asset('js/admin-notifications.js') }}?v={{ @filemtime(public_path('js/admin-notifications.js')) ?: time() }}"></script>
