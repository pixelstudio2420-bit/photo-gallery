  ...
  <div class="p-5">
    <form action="{{ route('photographer.events.update', $event) }}" method="POST" enctype="multipart/form-data">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        {{-- ── AI features (Face Search) — same toggle as create form.
             Pre-checks based on the existing event row so toggling
             OFF persists; the controller's update() honours
             $request->has(...) to distinguish "form omitted the field"
             from "user unticked it". ─────────────────────────── --}}
        <div class="md:col-span-2">
          <label class="block text-sm font-medium text-gray-700 mb-2">ฟีเจอร์ AI</label>
          <label class="flex items-start gap-3 p-4 rounded-xl border-2 border-gray-200 hover:border-indigo-300 cursor-pointer bg-white transition has-[:checked]:border-indigo-500 has-[:checked]:bg-indigo-50">
            <input type="checkbox" name="face_search_enabled" value="1"
                   class="peer sr-only"
                   {{ old('face_search_enabled', $event->face_search_enabled ? '1' : '0') === '1' ? 'checked' : '' }}>
