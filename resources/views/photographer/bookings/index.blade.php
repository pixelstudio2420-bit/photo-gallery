...
<div class="max-w-7xl mx-auto pb-16">
  ...
  @if(session('success'))
    <div class="mb-4 p-3.5 rounded-xl bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-200 dark:border-emerald-500/30 text-emerald-800 dark:text-emerald-200 flex items-start gap-2">
      <i class="bi bi-check-circle-fill mt-0.5 shrink-0"></i><span class="text-sm">{{ session('success') }}</span>
    </div>
  @endif
  @if(session('error'))
    <div class="mb-4 p-3.5 rounded-xl bg-rose-50 dark:bg-rose-500/10 border border-rose-200 dark:border-rose-500/30 text-rose-800 dark:text-rose-200 flex items-start gap-2">
      <i class="bi bi-exclamation-circle-fill mt-0.5 shrink-0"></i><span class="text-sm">{{ session('error') }}</span>
    </div>
  @endif
        ...
        @foreach($pending as $b)
          <div class="bk-pending-card">
            <div class="flex items-start justify-between gap-2 mb-2">
              <div class="min-w-0">
                <div class="font-bold text-sm text-slate-900 dark:text-white">{{ $b->title }}</div>
                <div class="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
                  <i class="bi bi-person"></i> {{ $b->customer?->first_name ?? '?' }}
                  @if($b->customer_phone) · <i class="bi bi-telephone"></i> {{ $b->customer_phone }} @endif
                </div>
              </div>
            ...
            <div class="text-xs text-slate-600 dark:text-slate-300 space-y-1 mb-3">
              <div><i class="bi bi-calendar-event mr-1"></i> {{ $b->scheduled_at->format('d/m/Y H:i') }} ({{ $b->duration_minutes }} นาที)</div>
              @if($b->location)<div><i class="bi bi-geo-alt mr-1"></i> {{ Str::limit($b->location, 60) }}</div>@endif
              @if($b->agreed_price)<div><i class="bi bi-cash-coin mr-1 text-emerald-500"></i> {{ number_format($b->agreed_price) }} ฿</div>@endif
            </div>
            <div class="flex items-center gap-2 flex-wrap">
              <form action="{{ route('photographer.bookings.confirm', $b->id) }}" method="POST" class="inline">
                @csrf
              ...
              <a href="{{ route('photographer.bookings.show', $b->id) }}" class="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-medium bg-slate-100 dark:bg-white/5 text-slate-700 dark:text-slate-200 hover:bg-slate-200 dark:hover:bg-white/10 transition no-underline">
    ...
    <div class="mt-5 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/10 overflow-hidden">
      ...
      <div class="divide-y divide-slate-100 dark:divide-white/5">
        @foreach($upcoming as $b)
          <a href="{{ route('photographer.bookings.show', $b->id) }}" class="flex items-center gap-3 px-4 py-3 hover:bg-slate-50 dark:hover:bg-white/[0.03] transition no-underline">
            <div class="w-12 h-12 rounded-xl flex flex-col items-center justify-center text-white shrink-0" style="background:linear-gradient(135deg,{{ $b->color }},{{ $b->color }}99);">
              <div class="text-[9px] uppercase font-bold opacity-80">{{ $b->scheduled_at->format('M') }}</div>
              <div class="text-base font-extrabold leading-none">{{ $b->scheduled_at->format('d') }}</div>
            </div>
            <div class="flex-1 min-w-0">
              <div class="font-semibold text-sm text-slate-900 dark:text-white truncate">{{ $b->title }}</div>
              <div class="text-[11px] text-slate-500 dark:text-slate-400 mt-0.5 flex items-center gap-2 flex-wrap">
                <span><i class="bi bi-clock"></i> {{ $b->scheduled_at->format('H:i') }}</span>
                <span><i class="bi bi-person"></i> {{ $b->customer?->first_name ?? '?' }}</span>
