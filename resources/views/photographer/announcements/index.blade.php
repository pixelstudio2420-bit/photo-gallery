...
@section('title', 'ประกาศสำหรับช่างภาพ')

@section('content')
<div class="max-w-5xl mx-auto p-6">
    <div class="mb-6">
        <h1 class="text-3xl font-bold text-slate-900 dark:text-white">📢 ประกาศ &amp; ข่าวสาร</h1>
        <p class="text-slate-500 dark:text-slate-400 mt-1">ข้อมูลอัปเดตและกิจกรรมสำหรับช่างภาพ</p>
    </div>
    ...
    @else
        <div class="grid gap-4 md:grid-cols-2">
            @foreach($announcements as $a)
                <a href="{{ route('photographer.announcements.show', $a->slug) }}"
                   class="bg-white dark:bg-slate-800 rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition group">
                    ...
                    @else
                        <div class="w-full h-44 bg-gradient-to-br from-indigo-500 to-violet-600 flex items-center justify-center text-white text-5xl">
                    ...
                    <div class="p-4">
                        ...
                        <h3 class="text-lg font-bold text-slate-900 dark:text-white group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition mb-2">
                            {{ $a->title }}
                        </h3>
                        @if($a->excerpt)
