...
<head>
  ...
  <style>
    ...
    /* Provider card hover lift — smooth + subtle shadow change. */
    .provider-card {
    ...
    /* Recommended ribbon — anchored top-right of LINE card. */
    .ribbon {
...
<body class="cg-bg min-h-screen flex items-center justify-center py-8 px-4">

  <div class="relative z-10 w-full max-w-[700px] cg-card rounded-3xl shadow-[0_30px_60px_-20px_rgba(0,0,0,0.6)] anim-in">
    <div class="p-6 sm:p-8 md:p-10">

      {{-- ── Hero ─────────────────────────────────────────────────── --}}
      <header class="text-center mb-7 anim-in anim-in-1">
        <div class="inline-flex items-center gap-2 mb-4">
          <div class="w-12 h-12 rounded-2xl flex items-center justify-center shadow-lg" style="background:linear-gradient(135deg,#06C755,#00b04f);box-shadow:0 10px 24px -6px rgba(6,199,85,0.5);">
            <i class="bi bi-line text-white text-2xl"></i>
          </div>
          <span class="text-slate-500 text-xl font-light px-1">หรือ</span>
          <div class="w-12 h-12 rounded-2xl flex items-center justify-center shadow-lg bg-white">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-5">
        ...
        <a href="{{ route('photographer.auth.redirect', ['provider' => 'line']) }}"
           class="provider-card anim-in anim-in-2 relative block p-5 rounded-2xl border pulse-line"
          ...
          <div class="flex items-center gap-3 mb-4">
            ...
            <div>
              <div class="text-white font-extrabold text-lg leading-tight">LINE</div>
              <div class="text-emerald-300/80 text-[11.5px] font-medium">เหมาะกับช่างภาพไทย</div>
            </div>
          </div>
          ...
          <div class="text-center py-2.5 rounded-xl text-white font-bold text-sm shadow-lg flex items-center justify-center gap-2"
               style="background:linear-gradient(135deg,#06C755 0%,#00b04f 100%);box-shadow:0 8px 24px -6px rgba(6,199,85,0.55);">
            <i class="bi bi-line text-base"></i> เชื่อมต่อ LINE
          </div>
        </a>

        {{-- ───────── Google ───────── --}}
        <a href="{{ route('photographer.auth.redirect', ['provider' => 'google']) }}"
           class="provider-card anim-in anim-in-3 relative block p-5 rounded-2xl border pulse-google"
          ...
          <div class="flex items-center gap-3 mb-4">
            ...
            <div>
              <div class="text-white font-extrabold text-lg leading-tight">Google</div>
              <div class="text-blue-200/80 text-[11.5px] font-medium">workflow + productivity</div>
            </div>
          </div>
      ...
      <div class="trust rounded-xl px-4 py-3 mb-5 anim-in anim-in-4">
        <div class="flex items-start gap-3">
          <i class="bi bi-info-circle-fill text-amber-400 text-base mt-0.5"></i>
          <div class="text-[12.5px] text-slate-300 leading-relaxed">
            ...
            <span class="block text-slate-500 mt-1 text-[11.5px]">
      ...
      {{-- ── Privacy note ─────────────────────────────────────────── --}}
      <div class="text-center text-[11.5px] text-slate-500 mb-5 anim-in anim-in-4">
        <i class="bi bi-shield-lock-fill"></i>
        ขออนุญาตเฉพาะข้อมูลที่จำเป็น · ไม่อ่านอีเมล/ไฟล์/แชทส่วนตัว · เพิกถอนได้ทุกเมื่อ
      </div>

      {{-- ── Footer ───────────────────────────────────────────────── --}}
      <div class="border-t border-white/[0.07] pt-4 mt-2 text-center anim-in anim-in-4">
        @if($userEmail)
          <div class="text-[11.5px] text-slate-500 mb-2">
            ลงชื่อเข้าใช้เป็น
        ...
        <form method="POST" action="{{ route('photographer.logout') }}" class="inline">
          @csrf
          <button type="submit" class="text-[12px] text-slate-500 hover:text-rose-400 transition inline-flex items-center gap-1.5">
