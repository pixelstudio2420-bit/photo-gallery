...
<div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-100 dark:border-white/[0.06]" id="online-users-card">
  ...
  <div class="p-0">
    <div id="online-users-table-wrapper">
      @if(($onlineUsers ?? collect())->isEmpty())
                ...
                <td class="px-4 py-3">
                  <div class="flex items-center gap-2">
                    <span class="inline-flex items-center justify-center shrink-0"
                      ...
                      {{ mb_strtoupper(mb_substr($u->first_name ?? 'U', 0, 1, 'UTF-8'), 'UTF-8') }}
                    </span>
                    <div>
                      <div class="font-medium leading-tight">
                        {{ trim(($u->first_name ?? '') . ' ' . ($u->last_name ?? '')) ?: 'User #'.$u->user_id }}
                      </div>
                      <div class="text-gray-500" style="font-size:0.72rem;">{{ $u->email ?? '' }}</div>
                    </div>
                  </div>
                </td>
                <td class="px-3 py-3">
                  ...
                  <span class="inline-flex items-center gap-1">
                    <i class="bi {{ $deviceIcon }}" style="color:#6b7280;"></i>
                    <span class="capitalize">{{ $u->device_type ?? 'desktop' }}</span>
                  </span>
                </td>
                <td class="px-3 py-3">
                  <span class="font-medium">{{ $u->browser ?? 'Unknown' }}</span>
                  <span class="text-gray-500"> / {{ $u->os ?? 'Unknown' }}</span>
                </td>
                <td class="px-3 py-3 text-gray-500">
                  {{ $u->last_activity ? \Carbon\Carbon::parse($u->last_activity)->diffForHumans() : '—' }}
                </td>
              </tr>
              @endforeach
            </tbody>
