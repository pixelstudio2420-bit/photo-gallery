...
@section('title', 'จัดการหมวดหมู่บล็อก')
...
<div x-data="categoryManager()">
    ...
    <div class="bg-white dark:bg-slate-800 rounded-2xl border border-gray-100 dark:border-white/[0.06] p-5 mb-6"
        ...
        <div x-show="showQuick" x-collapse x-cloak class="mt-4">
            <form method="POST" action="{{ route('admin.blog.categories.store') }}" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                @csrf
                <div>
                    ...
                    <tr class="hover:bg-gray-50/50 dark:hover:bg-slate-700/30 transition-colors"
                        ...
                        <td class="px-4 py-3">
                            <div class="flex items-center justify-center">
                                <div class="w-10 h-10 rounded-xl flex items-center justify-center"
                                    ...
                                    <i class="bi bi-{{ $category->icon ?? 'folder' }} text-lg"
                        ...
                        <td class="px-4 py-3">
                            <template x-if="!editing">
                                <span class="font-semibold text-slate-800 dark:text-white">{{ $category->name }}</span>
                            </template>
                            <template x-if="editing">
                                <form method="POST" action="{{ route('admin.blog.categories.update', $category) }}" class="flex items-center gap-2">
                                    @csrf @method('PUT')
                        ...
                        {{-- Slug --}}
                        <td class="px-4 py-3">
                            <code class="text-xs text-gray-500 dark:text-gray-400 bg-gray-100 dark:bg-slate-700 px-2 py-1 rounded">{{ $category->slug }}</code>
                        </td>

                        {{-- Parent --}}
                        <td class="px-4 py-3 text-center">
                            @if($category->parent)
                                <span class="text-xs text-gray-600 dark:text-gray-300">{{ $category->parent->name }}</span>
                            @else
                        ...
                        <td class="px-4 py-3 text-center">
                            <div class="flex items-center justify-center gap-2">
                                ...
                                <a href="{{ route('admin.blog.categories.edit', $category) }}" title="แก้ไขทั้งหมด"
                                ...
                                <button @click="deleteCategory({{ $category->id }}, '{{ $category->name }}')" title="ลบ"
    ...
    <form id="deleteCatForm" method="POST" class="hidden">@csrf @method('DELETE')</form>
</div>
@endsection
        ...
        deleteCategory(id, name) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    form.action = `{{ url('admin/blog/categories') }}/${id}`;
                    form.submit();
                }
            });
        }
