...
<div class="flex justify-between items-center mb-4">
  <h4 class="font-bold mb-0 tracking-tight">
    <i class="bi bi-pencil mr-2 text-indigo-500"></i>แก้ไขหมวดหมู่
  </h4>
  ...
  <div class="bg-white rounded-xl shadow-sm border border-gray-100">
    <div class="p-5">
      <div class="mb-3">
        ...
        <input type="text" name="name" class="w-full px-4 py-2.5 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500" value="{{ old('name', $category->name) }}" required>
      </div>
      <div class="mb-3">
        <label class="block text-sm font-semibold text-gray-700 mb-1.5">Slug</label>
        <input type="text" name="slug" class="w-full px-4 py-2.5 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500" value="{{ old('slug', $category->slug) }}">
      </div>
      <div class="mb-3">
        <label class="block text-sm font-semibold text-gray-700 mb-1.5">Icon</label>
        <input type="text" name="icon" class="w-full px-4 py-2.5 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500" value="{{ old('icon', $category->icon) }}" placeholder="e.g. camera, image, star">
      ...
      <button type="submit" class="bg-gradient-to-r from-indigo-500 to-indigo-600 text-white rounded-lg font-semibold px-6 py-2.5 transition hover:from-indigo-600 hover:to-indigo-700 inline-flex items-center gap-1">
        <i class="bi bi-check-lg mr-1"></i> อัพเดท
      </button>
    </div>
  </div>
</form>
