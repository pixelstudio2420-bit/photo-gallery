...
@section('title', 'เพิ่มราคา')

@section('content')
<div class="flex justify-between items-center mb-4">
  <h4 class="font-bold mb-0" style="letter-spacing:-0.02em;">
    <i class="bi bi-tag mr-2" style="color:#6366f1;"></i>เพิ่มราคา
  </h4>
  ...
  <div class="p-5 p-4">
    <form method="POST" action="{{ route('admin.pricing.store') }}">
      ...
      <div class="mb-3">
        ...
        <select name="event_id" class="w-full px-4 py-2.5 border border-gray-300 rounded-lg text-sm bg-white focus:ring-2 focus:ring-indigo-500" style="border-radius:10px;border-color:#e2e8f0;">
        ...
        @error('event_id') <div class="text-red-600 small mt-1">{{ $message }}</div> @enderror
      </div>
      <div class="mb-3">
        <label class="block text-sm font-medium text-gray-700 mb-1.5 font-medium">ราคาต่อรูป (บาท)</label>
        <input type="number" name="price_per_photo" class="w-full px-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500" style="border-radius:10px;border-color:#e2e8f0;" placeholder="0.00" step="0.01" min="0" value="{{ old('price_per_photo') }}">
