...
class FaceSearchService
{
    ...
    public function indexPhoto(\App\Models\EventPhoto $photo, ?string $imagePath = null): ?string
    {
        ...
        // 4) PLAN GATE — atomically consume one AI credit for this
        //    photographer BEFORE any AWS call. This is race-safe: two
        //    concurrent face-index jobs serialise through PlanGate's
        //    Cache::lock, so a photographer with N credits remaining
        //    can never have >N parallel index calls succeed against
        //    AWS. If the AWS call fails downstream we refund the
        //    credit via PlanGate::refundAiCredit (see step 5b).
        //
        //    Skipping here means:
        ...
        if ($photographerId > 0) {
            $consume = \App\Support\PlanGate::consumeAiCredit(
                $photographerId,
                'ai.face_index',
                ['photo_id' => $photo->id, 'event_id' => $photo->event_id],
            );
            if (!$consume['ok']) {
                Log::info("FaceSearchService::indexPhoto blocked by plan gate for photo #{$photo->id}", [
                    'photographer_id' => $photographerId,
                    'reason'          => $consume['reason'],
                    'used'            => $consume['used']      ?? null,
                    'cap'             => $consume['cap']       ?? null,
                ]);
                return null;
            }
            $creditConsumed = true;
        ...
        if (!$faceId) {
            // No face detected (legitimate) OR API error (network / IAM).
            // For the no-face case we REFUND the credit — it's wasteful to
            // charge the photographer for a photo that had no face to index.
            // For an API error we ALSO refund (the indexing didn't happen,
            // we'll retry later). Either way, the upfront-consumption
            // pattern in step 4 means we always bill 0 for blank attempts.
            if ($creditConsumed) {
                \App\Support\PlanGate::refundAiCredit(
                    $photographerId,
                    'ai.face_index',
                    ['photo_id' => $photo->id, 'event_id' => $photo->event_id],
                );
