<?php $__env->startSection('title', $event->name ?? 'Event'); ?>

<?php $__env->startSection('og-meta'); ?>
<?php echo $__env->make('layouts.partials.og-meta', [
...
<div class="event-hero">
  <div class="hero-container max-w-7xl mx-auto px-4 sm:px-6">
    ...
    <nav class="flex items-center gap-0.5 flex-wrap" style="margin-bottom:1rem;">
      <a href="<?php echo e(url('/')); ?>" style="color:rgba(255,255,255,0.45);text-decoration:none;font-size:0.8rem;transition:color 0.2s;"><i class="bi bi-house-door"></i></a>
      <span style="color:rgba(255,255,255,0.25);margin:0 0.5rem;font-size:0.7rem;"><i class="bi bi-chevron-right"></i></span>
      <a href="<?php echo e(route('events.index')); ?>" style="color:rgba(255,255,255,0.45);text-decoration:none;font-size:0.8rem;transition:color 0.2s;">อีเวนต์</a>
      <?php if($event->category): ?>
      <span style="color:rgba(255,255,255,0.25);margin:0 0.5rem;font-size:0.7rem;"><i class="bi bi-chevron-right"></i></span>
      <a href="<?php echo e(route('events.index')); ?>?category=<?php echo e($event->category->id); ?>" style="color:rgba(255,255,255,0.45);text-decoration:none;font-size:0.8rem;transition:color 0.2s;"><?php echo e($event->category->name); ?></a>
      <?php endif; ?>
      <span style="color:rgba(255,255,255,0.25);margin:0 0.5rem;font-size:0.7rem;"><i class="bi bi-chevron-right"></i></span>
      <span style="color:rgba(255,255,255,0.3);font-size:0.8rem;"><?php echo e(Str::limit($event->name, 40)); ?></span>
    </nav>
    ...
    <div style="display:flex;align-items:center;gap:0.75rem;margin-top:0.75rem;">
      ...
      <h1 style="color:#fff;font-weight:800;font-size:1.75rem;letter-spacing:-0.025em;line-height:1.2;margin:0;"><?php echo e($event->name); ?></h1>
    </div>

    <?php if($event->description): ?>
    <p style="color:rgba(255,255,255,0.5);font-size:0.9rem;margin-top:0.5rem;max-width:540px;line-height:1.6;">
      <?php echo e(Str::limit($event->description, 180)); ?>
    ...
    <div style="display:flex;flex-wrap:wrap;align-items:center;gap:0.5rem;margin-top:1.25rem;">
      ...
      <span style="display:inline-flex;align-items:center;gap:0.4rem;color:rgba(255,255,255,0.55);font-size:0.78rem;background:rgba(255,255,255,0.06);padding:0.3rem 0.75rem;border-radius:999px;border:1px solid rgba(255,255,255,0.06);">
        ...
        <?php echo e($event->shoot_date->translatedFormat('j F Y')); ?>
      ...
      <?php endif; ?>
      <?php if($pgName): ?>
      <a href="<?php echo e(route('photographers.show', $event->photographer_id)); ?>" style="display:inline-flex;align-items:center;gap:0.4rem;color:rgba(255,255,255,0.55);font-size:0.78rem;background:rgba(255,255,255,0.06);padding:0.3rem 0.75rem;border-radius:999px;border:1px solid rgba(255,255,255,0.06);text-decoration:none;transition:all 0.2s;" onmouseover="this.style.background='rgba(255,255,255,0.12)';this.style.color='rgba(255,255,255,0.8)';" onmouseout="this.style.background='rgba(255,255,255,0.06)';this.style.color='rgba(255,255,255,0.55)';">
      ...
      <span style="display:inline-flex;align-items:center;gap:0.4rem;color:rgba(255,255,255,0.55);font-size:0.78rem;background:rgba(255,255,255,0.06);padding:0.3rem 0.75rem;border-radius:999px;border:1px solid rgba(255,255,255,0.06);" id="hero-photo-count">
        ...
        <span id="hero-photo-num"><?php echo e(count($photos)); ?></span> ภาพ
      </span>
    </div>

    
    <div style="display:flex;align-items:center;flex-wrap:wrap;gap:0.6rem;margin-top:1.25rem;">
      <?php if(!empty($prices) && $prices->count() > 0): ?>
      <div style="display:inline-flex;align-items:center;gap:0.5rem;background:linear-gradient(135deg,#6366f1,#8b5cf6);color:#fff;padding:0.5rem 1.25rem;border-radius:999px;font-weight:700;font-size:0.95rem;box-shadow:0 4px 16px rgba(99,102,241,0.35);">
        ...
        <span><?php echo e(number_format($prices->first()->price, 0)); ?> ฿ / รูป</span>
      </div>
      <?php elseif(isset($basePricePerPhoto) && $basePricePerPhoto > 0): ?>
      <div style="display:inline-flex;align-items:center;gap:0.5rem;background:linear-gradient(135deg,#6366f1,#8b5cf6);color:#fff;padding:0.5rem 1.25rem;border-radius:999px;font-weight:700;font-size:0.95rem;box-shadow:0 4px 16px rgba(99,102,241,0.35);">
        <i class="bi bi-tag-fill" style="opacity:0.85;"></i>
        <span>เริ่มต้น <?php echo e(number_format($basePricePerPhoto, 2)); ?> ฿ / รูป</span>
      </div>
      <?php elseif($event->is_free ?? false): ?>
      ...
      <?php if($event->face_search_enabled ?? true): ?>
                ...
                <div class="login-modal-wrap" x-show="loginPrompt">
                  <div class="login-modal" @click.stop>
                    ...
                    <div class="login-modal-actions">
                      ...
                      <a href="<?php echo e(route('events.face-search', $event->id)); ?>" class="login-modal-btn primary">
      ...
      <div x-data="{
              ...
              shareUrl: '<?php echo e(route('events.show', $event->slug ?: $event->id)); ?>',
              shareTitle: <?php echo \Illuminate\Support\Js::from($event->name)->toHtml() ?>,
              init() {
            ...
            <div class="share-modal-wrap" x-show="shareOpen">
              <div class="share-modal" @click.stop>
                ...
                <div class="share-modal-grid">
                  <a href="https://www.facebook.com/sharer/sharer.php?u=<?php echo e(urlencode(route('events.show', $event->slug ?: $event->id))); ?>"
                  ...
                  <a href="https://social-plugins.line.me/lineit/share?url=<?php echo e(urlencode(route('events.show', $event->slug ?: $event->id))); ?>"
                     target="_blank" rel="noopener"
                     class="share-target"
                     @click="closeShare()">
                  ...
                  <a href="https://twitter.com/intent/tweet?url=<?php echo e(urlencode(route('events.show', $event->slug ?: $event->id))); ?>&text=<?php echo e(urlencode($event->name)); ?>"
                     target="_blank" rel="noopener"
                     class="share-target"
                     @click="closeShare()">
                  ...
                  <a href="https://api.whatsapp.com/send?text=<?php echo e(urlencode($event->name . ' ' . route('events.show', $event->slug ?: $event->id))); ?>"
                     target="_blank" rel="noopener"
                     class="share-target"
                     @click="closeShare()">
                  ...
                  <a href="https://t.me/share/url?url=<?php echo e(urlencode(route('events.show', $event->slug ?: $event->id))); ?>&text=<?php echo e(urlencode($event->name)); ?>"
                     target="_blank" rel="noopener"
                     class="share-target"
                     @click="closeShare()">
                  ...
                  <a href="mailto:?subject=<?php echo e(rawurlencode($event->name)); ?>&body=<?php echo e(rawurlencode(route('events.show', $event->slug ?: $event->id))); ?>"
                     class="share-target"
                     @click="closeShare()">
  ...
  <div class="max-w-7xl mx-auto px-4">
    <div class="flex items-center gap-2 overflow-auto pb-1" style="scrollbar-width:none;">
      ...
      <button class="pkg-chip package-btn inline-flex items-center gap-1.5 bg-white dark:bg-slate-800 border-[1.5px] border-slate-200 dark:border-white/10 px-4 py-2 rounded-xl text-xs font-semibold text-slate-700 dark:text-slate-300 whitespace-nowrap cursor-pointer transition-all hover:border-indigo-400 hover:text-indigo-600 hover:-translate-y-0.5 hover:shadow-md hover:shadow-indigo-500/10 shrink-0"
          ...
          data-count="<?php echo e($pkg->photo_count); ?>"
          data-price="<?php echo e($pkg->price); ?>">
        <?php echo e($pkg->name); ?>

        <span class="text-gray-400">—</span>
        <?php echo e($pkg->photo_count); ?> รูป
        <span class="text-indigo-500 font-bold"><?php echo e(number_format($pkg->price, 0)); ?> ฿</span>
      </button>
    ...
    <div class="flex items-center justify-between gap-3 py-2">
      ...
      <div class="flex items-center gap-2 shrink-0">
        ...
        <div class="hidden md:flex items-center gap-0.5 bg-white/[0.04] rounded-full p-1 border border-white/[0.06]">
          <?php $__currentLoopData = [3,4,5,6,8]; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $c): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
          <button class="cb col-btn w-7 h-7 flex items-center justify-center rounded-full text-[11px] font-bold border-0 bg-transparent text-slate-500 cursor-pointer transition-all duration-200 hover:bg-white/10 hover:text-indigo-300 <?php echo e($c === 5 ? 'toolbar-col-active' : ''); ?>" data-col="<?php echo e($c); ?>" title="<?php echo e($c); ?> คอลัมน์"><?php echo e($c); ?></button>
          <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
        </div>
      </div>
  ...
  <div style="position:absolute;inset:0;display:flex;align-items:center;justify-content:center;z-index:5;padding:56px 70px 190px;pointer-events:none;" id="lb-image-area">
    ...
    <?php if($lbWmImageUrl): ?>
      <div class="lb-wm-layer lb-wm-image-layer"
           style="background-image:url('<?php echo e($lbWmImageUrl); ?>');opacity:<?php echo e(number_format($lbWmOpacity, 2)); ?>;"></div>
    <?php elseif($lbWmText): ?>
      <div class="lb-wm-layer lb-wm-text-layer"
           style="color:<?php echo e($wmColor); ?>;opacity:<?php echo e(number_format(min(0.35, $lbWmOpacity + 0.05), 2)); ?>;">
        <?php for($__wmi = 0; $__wmi < 24; $__wmi++): ?><span><?php echo e($lbWmText); ?></span><?php endfor; ?>
      </div>
    <?php endif; ?>
  </div>
...
const PRICE_PER  = <?php echo e(!empty($prices) && $prices->count() > 0 ? $prices->first()->price : ($event->price_per_photo ?? 0)); ?>;
const HAS_PACKAGES = <?php echo e(isset($packages) && $packages->count() > 0 ? 'true' : 'false'); ?>;
const DRIVE_FOLDER = <?php echo json_encode($event->drive_folder_id ?? '', 15, 512) ?>;
...
const EVENT_SLUG  = <?php echo json_encode($event->slug ?? (string)$event->id, 15, 512) ?>;

const SERVER_PHOTOS = <?php echo json_encode($photos ?? [], 15, 512) ?>;
...
const GALLERY_EAGER_COUNT = <?php echo e(max(0,  min(60,  (int) \App\Models\AppSetting::get('photo_gallery_eager_count', 12)))); ?>;
const GALLERY_THUMB_SIZE  = <?php echo e(max(100, min(600, (int) \App\Models\AppSetting::get('photo_gallery_thumb_size', 200)))); ?>;
...
function promptLogin() {
  ...
  }).then(result => { if (result.isConfirmed) window.location.href = '<?php echo e(route("login")); ?>'; });
}
...
async function goToCart() {
  ...
  try {
    ...
    if (res.ok) { window.location.href = '<?php echo e(route("cart.index")); ?>'; }
...
<section class="bg-white dark:bg-slate-900 border-t border-slate-200 dark:border-white/[0.06] py-12">
  <div class="max-w-5xl mx-auto px-4">
    <div class="flex items-end justify-between mb-6">
      ...
      <?php if($reviewStats['total'] > 10): ?>
        <a href="<?php echo e(route('reviews.index', ['event_id' => $event->id])); ?>" class="text-sm text-indigo-600 hover:text-indigo-700 font-medium">
          ดูทั้งหมด <?php echo e($reviewStats['total']); ?> รีวิว →
        </a>
      <?php endif; ?>
    </div>

    <?php echo $__env->make('public.reviews._section', ['reviewStats' => $reviewStats, 'reviewsList' => $reviewsList, 'showEvent' => false], array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
  </div>
</section>
<?php endif; ?>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\photo-gallery-pgsql\resources\views/public/events/show.blade.php ENDPATH**/ ?>