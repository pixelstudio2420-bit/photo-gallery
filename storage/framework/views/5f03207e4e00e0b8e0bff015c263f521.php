<?php $__env->startSection('title', 'สินค้าดิจิทัล · Digital Products'); ?>
  ...
  <div class="relative max-w-6xl mx-auto px-4 py-14 md:py-20">
    ...
    <nav aria-label="Breadcrumb" class="mb-6 max-w-2xl mx-auto">
      <ol class="flex items-center justify-center gap-2 text-xs flex-wrap">
        <li>
          <a href="<?php echo e(url('/')); ?>" class="text-slate-600 dark:text-slate-400 hover:text-indigo-600 dark:hover:text-indigo-300 transition-colors">
      ...
      <div class="relative group">
        ...
        <div class="relative flex items-center bg-white dark:bg-slate-900
          ...
          <button type="button" id="hero-clear-btn"
                  class="<?php echo e(!empty($search) ? '' : 'hidden'); ?> mr-2 p-2 rounded-lg
    ...
    <div class="flex items-center justify-center gap-2 mt-7 overflow-x-auto pb-1 -mx-1 px-1"
      ...
      <button class="hero-type-chip active-chip shrink-0 inline-flex items-center gap-1.5 px-4 py-2 rounded-full
        ...
        <span class="opacity-80">(<?php echo e($totalCount); ?>)</span>
      </button>
      <?php $__currentLoopData = $typeLabels; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key => $meta): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
        <?php $count = $typeCounts[$key] ?? 0; ?>
        <?php if($count > 0): ?>
        <button class="hero-type-chip shrink-0 inline-flex items-center gap-1.5 px-4 py-2 rounded-full
                ...
                onclick="window.dispatchEvent(new CustomEvent('hero-type', {detail:{id:'<?php echo e($key); ?>'}}))">
          <i class="bi <?php echo e($meta['icon']); ?>"></i> <?php echo e($meta['label']); ?>

          <span class="opacity-70">(<?php echo e($count); ?>)</span>
        </button>
        <?php endif; ?>
    ...
    <div class="flex items-center justify-center gap-2.5 mt-6 flex-wrap">
      <div class="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full
        ...
        <span class="text-xs font-medium text-slate-600 dark:text-slate-400">
          ทั้งหมด <strong class="text-slate-900 dark:text-slate-50" id="stat-total"><?php echo e($totalCount); ?></strong> รายการ
        </span>
      </div>
      <?php if($onSaleCount > 0): ?>
      <div class="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full
        ...
        <span class="text-xs font-semibold text-rose-700 dark:text-rose-300">
          <strong><?php echo e($onSaleCount); ?></strong> กำลังลดราคา
        </span>
      </div>
      <?php endif; ?>
      <div class="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full
...
<div class="-mx-4 md:-mx-6 lg:-mx-8 bg-slate-50 dark:bg-slate-950">
  <div class="max-w-7xl mx-auto px-4 md:px-6 lg:px-8 py-8 md:py-10">
    ...
    <?php if($featured->count() > 0 && empty($search) && (empty($type) || $type === 'all')): ?>
    <section class="mb-10">
      <div class="rounded-3xl bg-white dark:bg-slate-900
        ...
        <div class="flex items-center justify-between mb-5">
          ...
          <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full
            ...
            <i class="bi bi-lightning-charge-fill"></i> <?php echo e($featured->count()); ?> รายการเด็ด
          </span>
        </div>
        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 md:gap-4">
          ...
          <a href="<?php echo e(route('products.show', $f->slug)); ?>"
            ...
            <?php if($f->cover_image_url): ?>
              <img src="<?php echo e($f->cover_image_url); ?>" alt="<?php echo e($f->name); ?>"
            ...
            <div class="absolute bottom-0 left-0 right-0 p-3">
              <div class="text-white text-xs font-semibold line-clamp-1 mb-0.5 drop-shadow"><?php echo e($f->name); ?></div>
              <div class="text-white text-[13px] font-bold drop-shadow flex items-center gap-1">
                <?php if($f->sale_price): ?>
                  <span class="text-rose-300"><?php echo e(number_format($f->sale_price, 0)); ?> ฿</span>
                  <span class="text-white/60 text-[10px] line-through font-medium"><?php echo e(number_format($f->price, 0)); ?></span>
                <?php else: ?>
                  <?php echo e(number_format($f->price, 0)); ?> ฿
                <?php endif; ?>
              </div>
            </div>
            <?php if($f->sale_price): ?>
    ...
    <div x-data="productSearch()" x-init="init()"
      ...
      <div x-show="!loading" x-transition.opacity>
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5" id="products-grid">
          <?php echo $__env->make('public.products._grid', ['products' => $products, 'typeLabels' => $typeLabels], array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
        </div>
        ...
        <div id="products-pagination" class="mt-12">
          <?php echo $__env->make('public.products._pagination', ['products' => $products], array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
        </div>
      </div>
    </div>
  </div>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 md:gap-5">
        ...
        <div class="group rounded-2xl p-5
          ...
          <div class="flex items-start gap-3">
            <div class="w-12 h-12 flex-shrink-0 rounded-xl bg-gradient-to-br <?php echo e($b['grad']); ?> text-white
                        flex items-center justify-center shadow-md group-hover:scale-110 transition-transform duration-300">
              <i class="bi <?php echo e($b['icon']); ?> text-xl"></i>
            </div>
            <div>
              <h4 class="font-bold text-sm text-slate-900 dark:text-slate-50 mb-1"><?php echo e($b['title']); ?></h4>
              <p class="text-xs text-slate-500 dark:text-slate-400 leading-relaxed"><?php echo e($b['sub']); ?></p>
            </div>
          </div>
        </div>
        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
...
function productSearch() {
  return {
    query:   <?php echo json_encode($search ?? ''); ?>,
    type:    <?php echo json_encode($type ?: ''); ?>,
    sort:    <?php echo json_encode($sort ?: 'latest'); ?>,
    onSale:  <?php echo e(request()->boolean('on_sale') ? 'true' : 'false'); ?>,
    loading: false,
    total:   <?php echo e($products->total()); ?>,
    showing: <?php echo e($products->count()); ?>,
    ...
    async fetchProducts(page) {
      ...
      try {
        const res  = await fetch(`<?php echo e(route('products.index')); ?>?${params.toString()}`, {
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\photo-gallery-pgsql\resources\views/public/products/index.blade.php ENDPATH**/ ?>