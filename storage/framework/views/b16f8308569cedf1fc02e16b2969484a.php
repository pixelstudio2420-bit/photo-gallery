<?php $__env->startSection('title', 'เลือกแผนสมัครสมาชิก'); ?>
...
<div class="plans-bg" x-data="plansPage()" x-cloak>
  ...
  <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-5 gap-5 max-w-8xl mx-auto">
    <?php $__currentLoopData = $plans; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $idx => $p): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
      <?php
      ...
      <div class="plan-tile plan-anim <?php echo e($delayClass); ?> <?php echo e($isCurrent ? 'current' : ($isPopular ? 'popular' : '')); ?>"
           style="--accent: <?php echo e($accent); ?>; --accent-soft: <?php echo e($accent); ?>1f; --accent-border: <?php echo e($accent); ?>35; --accent-shadow: <?php echo e($accent); ?>55;">
        <?php if($isCurrent): ?>
        ...
        <div class="plan-header">
          <div class="plan-icon"><i class="bi <?php echo e($icon); ?>"></i></div>
          <h3 class="plan-name"><?php echo e($p->name); ?></h3>
          <?php if($p->tagline): ?>
            <p class="plan-tagline"><?php echo e($p->tagline); ?></p>
          <?php endif; ?>
          ...
          <div class="plan-price">
            ...
            <?php else: ?>
              ...
              <span class="plan-price-value">
                ...
                <span x-text="annual
                  ? numberFmt(Math.round((<?php echo e((float) $p->price_annual_thb); ?> || (<?php echo e((float) $p->price_thb); ?>*12)) / 12))
                  : numberFmt(<?php echo e((float) $p->price_thb); ?>)"></span>
              </span>
              <span class="plan-price-suffix">/เดือน</span>

              <div class="mt-2 flex flex-col items-center gap-1.5">
                
                <template x-if="annual && <?php echo e($p->price_annual_thb ? 1 : 0); ?>">
                  <span class="plan-price-annual">
                    <i class="bi bi-tag-fill"></i>
                    เก็บปีละ ฿<span x-text="numberFmt(<?php echo e((float) ($p->price_annual_thb ?? 0)); ?>)"></span>
                    <?php if($p->annualSavings() > 0): ?>
                      <span class="text-emerald-700 dark:text-emerald-300 font-extrabold">· ประหยัด ฿<?php echo e(number_format($p->annualSavings(), 0)); ?></span>
                    <?php endif; ?>
                  </span>
                </template>
                
                <template x-if="!annual && <?php echo e($p->annualSavings() > 0 ? 1 : 0); ?>">
                  <span class="text-[11px] text-emerald-600 dark:text-emerald-400 font-semibold inline-flex items-center gap-1">
                    <i class="bi bi-arrow-up-right"></i>
                    เลือกรายปีประหยัด ฿<?php echo e(number_format($p->annualSavings(), 0)); ?>

                  </span>
                </template>

                <?php if($valuePerGb > 0 && $valuePerGb < 100): ?>
                  <span class="plan-value-tip">
                    <i class="bi bi-graph-down-arrow"></i>
                    ฿<?php echo e(number_format($valuePerGb, $valuePerGb < 1 ? 2 : 1)); ?> ต่อ GB
                  </span>
                <?php endif; ?>
              </div>
            <?php endif; ?>
        ...
        <div class="plan-body">
          ...
          <div class="plan-stats">
            <div class="plan-stat-item">
              ...
              <div class="plan-stat-val"><?php echo e(number_format($p->storage_gb, 0)); ?><small>GB</small></div>
            </div>
            <div class="plan-stat-item">
              <div class="plan-stat-icon"><i class="bi bi-percent"></i></div>
              <div class="plan-stat-label">ค่าคอม</div>
              <div class="plan-stat-val"><?php echo e(rtrim(rtrim(number_format((float) $p->commission_pct, 1), '0'), '.')); ?><small>%</small></div>
            </div>
            <div class="plan-stat-item">
              <div class="plan-stat-icon"><i class="bi bi-calendar-event"></i></div>
              <div class="plan-stat-label">อีเวนต์</div>
              <div class="plan-stat-val">
                <?php if(is_null($p->max_concurrent_events)): ?>
                  <span style="color:<?php echo e($accent); ?>">∞</span>
                <?php else: ?>
                  <?php echo e((int) $p->max_concurrent_events); ?>
          ...
          <div>
            <p class="plan-features-label">
              <i class="bi bi-magic" style="color:<?php echo e($accent); ?>"></i>
              <span>ฟีเจอร์ <span class="text-indigo-600 dark:text-indigo-400 font-extrabold"><?php echo e($featureCount); ?></span> รายการ</span>
            </p>
            <div class="plan-features">
              <?php $__currentLoopData = $featureLabels; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $code => [$label, $iconClass]): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <?php $enabled = in_array($code, $p->ai_features ?? [], true); ?>
                <div class="plan-feature-row <?php echo e($enabled ? '' : 'locked'); ?>">
                  <span class="plan-feature-icon <?php echo e($enabled ? 'on' : 'off'); ?>">
                    <i class="bi <?php echo e($enabled ? 'bi-check-lg' : 'bi-dash'); ?>"></i>
                  </span>
          ...
          <?php else: ?>
            <button type="button" @click="open(<?php echo e(Js::from($planPayload)); ?>)"
                    class="plan-cta <?php echo e($p->isFree() ? 'plan-cta-free' : ($isPopular ? 'plan-cta-primary' : 'plan-cta-secondary')); ?>">
              <?php if($p->isFree()): ?>
...
<?php echo $__env->make('layouts.photographer', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\photo-gallery-pgsql\resources\views/photographer/subscription/plans.blade.php ENDPATH**/ ?>