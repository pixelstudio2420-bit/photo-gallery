<?php $__env->startSection('title', 'AI Tools'); ?>
...
<div class="pg-card pg-card-padded mb-6 pg-anim d1">
  <div class="flex items-center justify-between mb-2">
    ...
    <span class="text-sm font-bold text-gray-900 dark:text-gray-100">
      <span class="is-mono"><?php echo e(number_format($creditsUsed)); ?></span>
      <span class="text-gray-400">/ <?php echo e(number_format($creditsCap)); ?></span>
    </span>
  </div>
  <div class="w-full bg-gray-100 dark:bg-white/[0.06] rounded-full h-2.5 overflow-hidden">
    ...
    ?>
    <div class="h-2.5 rounded-full transition-all" style="width: <?php echo e(min(100, $apct)); ?>%; background: <?php echo e($barGradient); ?>;"></div>
  </div>
  <p class="text-xs text-gray-500 dark:text-gray-400 mt-2">
    เหลือ <strong class="text-indigo-600 dark:text-indigo-300"><?php echo e(number_format($creditsRemaining)); ?></strong> เครดิต
...
<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
  <?php $__currentLoopData = $featureMeta; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $code => $meta): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
    <?php
    ...
    <button type="button"
            <?php if($enabled): ?>
              onclick="document.getElementById('event-picker').setAttribute('data-route', '<?php echo e(route('photographer.ai.'.$meta['route'], 0)); ?>'); document.getElementById('event-picker').setAttribute('data-label', '<?php echo e($meta['label']); ?>'); document.getElementById('event-picker-modal').classList.remove('hidden');"
            <?php else: ?>
              disabled
            <?php endif; ?>
            class="pg-card pg-card-padded text-left transition relative overflow-hidden
                   <?php echo e($enabled ? 'pg-card-hover cursor-pointer' : 'opacity-60 cursor-not-allowed'); ?>">
      
      <span class="absolute top-0 right-0 w-24 h-24 pointer-events-none rounded-tr-2xl"
            style="background: radial-gradient(circle at top right, <?php echo e($c1); ?>26 0%, transparent 70%);"></span>
      <div class="relative flex items-center gap-3 mb-2">
        <div class="w-10 h-10 rounded-xl flex items-center justify-center text-white shadow-sm shrink-0"
             style="background:linear-gradient(135deg, <?php echo e($c1); ?>, <?php echo e($c2); ?>);">
          <i class="bi <?php echo e($meta['icon']); ?> text-lg"></i>
        </div>
        <?php if(!$enabled): ?>
      ...
      <p class="relative font-bold text-sm text-gray-900 dark:text-gray-100"><?php echo e($meta['label']); ?></p>
      ...
      <table class="pg-table">
        ...
        <tbody>
          <?php $__currentLoopData = $recentTasks; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $task): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <tr>
              <td class="font-medium"><?php echo e($featureMeta[$task->kind]['label'] ?? $task->kind); ?></td>
              <td class="text-gray-600"><?php echo e($task->event?->name ?? '—'); ?></td>
              <td>
                <?php
                ...
                ?>
                <span class="pg-pill <?php echo e($badge[0]); ?>"><?php echo e($badge[1]); ?></span>
              </td>
              <td class="text-end is-mono"><?php echo e(number_format($task->credits_used)); ?></td>
              <td class="text-end text-xs text-gray-500"><?php echo e($task->created_at?->diffForHumans()); ?></td>
            </tr>
          <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
        </tbody>
      </table>
...
<?php echo $__env->make('layouts.photographer', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\photo-gallery-pgsql\resources\views/photographer/ai/index.blade.php ENDPATH**/ ?>