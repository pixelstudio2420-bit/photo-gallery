<?php $__env->startSection('title', 'Bookings — คิวงานทั้งระบบ'); ?>
...
<div class="max-w-[1400px] mx-auto pb-12">
    ...
    <div class="flex items-center justify-between gap-4 flex-wrap">
      ...
      <div class="flex items-center gap-2 text-[11px]">
        <span class="px-3 py-1.5 rounded-full bg-white/15 backdrop-blur border border-white/30 font-bold">
          เดือนนี้: <?php echo e(number_format($stats['this_month'])); ?> bookings
        </span>
      </div>
    </div>
  </div>
  ...
  <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-3 mb-5">
    <div class="ad-stat">
      ...
      <div class="ad-stat-num text-slate-900 dark:text-white"><?php echo e(number_format($stats['total'])); ?></div>
    </div>
    <div class="ad-stat">
      <div class="ad-stat-label mb-1.5"><i class="bi bi-hourglass-split text-amber-500"></i> รอยืนยัน</div>
      <div class="ad-stat-num text-amber-600"><?php echo e(number_format($stats['pending'])); ?></div>
    </div>
    <div class="ad-stat">
      <div class="ad-stat-label mb-1.5"><i class="bi bi-check-circle-fill text-emerald-500"></i> ยืนยัน</div>
      <div class="ad-stat-num text-emerald-600"><?php echo e(number_format($stats['confirmed'])); ?></div>
    </div>
    <div class="ad-stat">
      <div class="ad-stat-label mb-1.5"><i class="bi bi-flag-fill text-indigo-500"></i> เสร็จแล้ว</div>
      <div class="ad-stat-num text-indigo-600"><?php echo e(number_format($stats['completed'])); ?></div>
    </div>
    <div class="ad-stat">
      <div class="ad-stat-label mb-1.5"><i class="bi bi-x-circle-fill text-rose-500"></i> ยกเลิก</div>
      <div class="ad-stat-num text-rose-600"><?php echo e(number_format($stats['cancelled'] + $stats['no_show'])); ?></div>
    </div>
    <div class="ad-stat">
      <div class="ad-stat-label mb-1.5"><i class="bi bi-percent text-cyan-500"></i> Completion</div>
      <div class="ad-stat-num text-cyan-600"><?php echo e($stats['completion_rate']); ?>%</div>
    </div>
    <div class="ad-stat">
      <div class="ad-stat-label mb-1.5"><i class="bi bi-cash-coin text-emerald-500"></i> รายรับ</div>
      <div class="ad-stat-num text-emerald-600">฿<?php echo e(number_format($stats['revenue_completed'])); ?></div>
    </div>
  </div>
  ...
  <form method="GET" class="rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/10 p-4 mb-4">
    <div class="grid grid-cols-1 md:grid-cols-5 gap-3">
      <div class="md:col-span-2">
        ...
        <input type="text" name="q" value="<?php echo e($filters['q'] ?? ''); ?>" placeholder="ชื่องาน / สถานที่ / อีเมลลูกค้า"
      ...
      <div>
        ...
        <select name="status" class="w-full px-3 py-2 rounded-lg border border-slate-200 dark:border-white/10 bg-white dark:bg-slate-800 text-sm">
          ...
          <?php $__currentLoopData = ['pending'=>'รอยืนยัน','confirmed'=>'ยืนยันแล้ว','completed'=>'เสร็จสิ้น','cancelled'=>'ยกเลิก','no_show'=>'ไม่มาตามนัด']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $k=>$v): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <option value="<?php echo e($k); ?>" <?php echo e(($filters['status']??'')===$k ? 'selected':''); ?>><?php echo e($v); ?></option>
    ...
    <div class="flex items-center gap-2 mt-3 flex-wrap">
      ...
      <a href="<?php echo e(route('admin.bookings.index')); ?>" class="text-xs text-slate-500 hover:text-slate-700">Clear</a>
      <span class="ml-auto text-xs text-slate-500">แสดง <?php echo e($bookings->count()); ?> จาก <?php echo e($bookings->total()); ?> รายการ</span>
    </div>
  </form>
  ...
  <div class="rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/10 overflow-hidden">
    ...
    <?php else: ?>
          ...
          <tbody class="divide-y divide-slate-100 dark:divide-white/5">
            <?php $__currentLoopData = $bookings; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $b): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
              <tr class="hover:bg-slate-50 dark:hover:bg-white/[0.02]">
                <td class="px-4 py-3 text-xs text-slate-500">#<?php echo e($b->id); ?></td>
                <td class="px-4 py-3">
                  <div class="font-semibold text-slate-900 dark:text-white"><?php echo e(Str::limit($b->title, 35)); ?></div>
                  <div class="text-[11px] text-slate-500"><?php echo e($b->customer?->first_name ?? '?'); ?> · <?php echo e($b->customer?->email ?? '-'); ?></div>
                </td>
                <td class="px-4 py-3">
                  <div class="text-slate-700 dark:text-slate-200"><?php echo e($b->photographerProfile?->display_name ?? $b->photographer?->first_name ?? '?'); ?></div>
                </td>
                <td class="px-4 py-3 text-xs">
                  <div class="font-semibold text-slate-700 dark:text-slate-200"><?php echo e($b->scheduled_at->format('d/m/Y')); ?></div>
                  <div class="text-slate-500"><?php echo e($b->scheduled_at->format('H:i')); ?> · <?php echo e($b->duration_minutes); ?>m</div>
                </td>
                <td class="px-4 py-3 text-right">
                  <?php if($b->agreed_price): ?>
                    <div class="font-bold text-emerald-600">฿<?php echo e(number_format($b->agreed_price)); ?></div>
                    <?php if($b->deposit_paid > 0): ?>
                      <div class="text-[10px] text-emerald-500">มัดจำ ฿<?php echo e(number_format($b->deposit_paid)); ?></div>
                    <?php endif; ?>
                  <?php else: ?>
                ...
                <td class="px-4 py-3 text-center">
                  ...
                  <?php if($b->is_waitlist): ?>
                    <div class="text-[9px] text-violet-600 mt-0.5">⏳ Waitlist</div>
                  <?php endif; ?>
                </td>
                <td class="px-4 py-3 text-right">
                  <a href="<?php echo e(route('admin.bookings.show', $b->id)); ?>" class="inline-flex items-center justify-center w-8 h-8 rounded-full bg-indigo-50 dark:bg-indigo-500/10 text-indigo-600 hover:bg-indigo-500 hover:text-white transition">
      ...
      <div class="px-4 py-3 border-t border-slate-100 dark:border-white/5"><?php echo e($bookings->links()); ?></div>
    <?php endif; ?>
  </div>
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\photo-gallery-pgsql\resources\views/admin/bookings/index.blade.php ENDPATH**/ ?>