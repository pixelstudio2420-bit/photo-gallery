<?php $__env->startSection('title', 'ทำไมต้องเลือกเรา · ขายรูปอีเวนต์ผ่าน LINE + AI หาใบหน้า'); ?>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-10 items-center">
      ...
      <div class="lg:col-span-7 text-white p-anim d1">
        ...
        <div class="flex flex-wrap items-center gap-3 mb-8">
          ...
          <a href="<?php echo e(route('photographer.register')); ?>"
        ...
        <div class="grid grid-cols-2 grid-rows-3 gap-3 h-[400px] md:h-[480px]">
          ...
          <div class="bento bento-shine row-span-2 p-4 flex flex-col justify-between">
            <div>
              <div class="flex items-center gap-2 mb-3">
                ...
                <div class="text-white text-xs font-bold"><?php echo e($siteName ?? config('app.name')); ?></div>
              </div>
              <div class="space-y-2">
          ...
          <div class="bento bento-shine col-span-2 p-3 flex items-center justify-around"
            ...
            <div class="text-center">
              <div class="text-xl md:text-2xl font-extrabold text-white" x-show="shown" x-transition><?php echo e(number_format($stats['photographers'])); ?>+</div>
              <div class="text-[9px] text-white/65 uppercase tracking-widest font-bold">ช่างภาพ</div>
            </div>
            <div class="w-px h-8 bg-white/15"></div>
            <div class="text-center">
              <div class="text-xl md:text-2xl font-extrabold text-white" x-show="shown" x-transition><?php echo e(number_format($stats['events'])); ?>+</div>
              <div class="text-[9px] text-white/65 uppercase tracking-widest font-bold">อีเวนต์</div>
            </div>
            <div class="w-px h-8 bg-white/15"></div>
            <div class="text-center">
              <div class="text-xl md:text-2xl font-extrabold text-white" x-show="shown" x-transition><?php echo e(number_format($stats['orders'])); ?>+</div>
...
<?php if($photographerPlans->count() > 0): ?>
<?php
...
<section class="mb-14 md:mb-20" id="pricing">
  ...
  <div class="grid grid-cols-1 md:grid-cols-3 gap-5 lg:gap-6 max-w-5xl mx-auto">
    <?php $__currentLoopData = $showcase; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $i => $plan): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
      <?php
        $features = is_string($plan->features_json) ? (json_decode($plan->features_json, true) ?: []) : ($plan->features_json ?: []);
        $isFeatured = $plan->code === 'pro';
      ?>
      <div class="pcard <?php echo e($isFeatured ? 'is-featured' : ''); ?> p-anim d<?php echo e($i+2); ?>">
        <div class="mb-4">
          <h3 class="text-xl font-bold text-slate-900 dark:text-white"><?php echo e($plan->name); ?></h3>
          <?php if($plan->tagline): ?>
            <p class="text-xs text-slate-500 dark:text-slate-400 mt-1 leading-snug"><?php echo e($plan->tagline); ?></p>
          <?php endif; ?>
        </div>

        <div class="mb-5 pb-5 border-b border-slate-200 dark:border-white/10">
          <div class="flex items-baseline gap-1">
            <span class="text-4xl font-extrabold text-slate-900 dark:text-white">฿<?php echo e(number_format($plan->price_thb)); ?></span>
            <span class="text-sm text-slate-500 dark:text-slate-400">/เดือน</span>
          </div>
          <?php if($plan->price_annual_thb): ?>
            <div class="text-[11px] text-emerald-600 dark:text-emerald-400 font-bold mt-1">
              <i class="bi bi-tag-fill"></i> ฿<?php echo e(number_format($plan->price_annual_thb)); ?>/ปี · ประหยัด 2 เดือน
            </div>
          <?php elseif($plan->price_thb == 0): ?>
            <div class="text-[11px] text-slate-500 dark:text-slate-400 font-medium mt-1">ฟรีตลอดชีพ</div>
          <?php endif; ?>
        </div>

        <ul class="space-y-2.5 mb-6 text-sm">
          <?php $__currentLoopData = array_slice($features, 0, 5); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $f): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <li class="flex items-start gap-2 text-slate-700 dark:text-slate-300">
              <i class="bi bi-check-lg <?php echo e($isFeatured ? 'text-violet-500 dark:text-violet-400' : 'text-emerald-500 dark:text-emerald-400'); ?> mt-0.5 shrink-0 text-base"></i>
              <span class="leading-snug"><?php echo e($f); ?></span>
            </li>
        ...
        <a href="<?php echo e(route('photographer.register')); ?>"
          ...
          <?php echo e($plan->price_thb == 0 ? 'เริ่มฟรีเลย' : 'เลือกแผนนี้'); ?>
  ...
  <?php if($photographerPlans->count() > 3): ?>
    <div class="text-center mt-6 p-anim d5">
      <a href="<?php echo e(route('photographer.register')); ?>#pricing"
         class="inline-flex items-center gap-1.5 text-sm font-semibold text-indigo-600 dark:text-indigo-400 hover:underline">
        ดูแผนทั้งหมด (<?php echo e($photographerPlans->count()); ?> แผน) <i class="bi bi-arrow-right"></i>
      </a>
    </div>
  <?php endif; ?>
</section>
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-3 md:gap-4 mt-10">
        ...
        <?php $__currentLoopData = $reviews; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $r): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
          <div class="bento p-4 text-white">
            ...
            <p class="text-xs text-white/85 leading-relaxed mb-3">"<?php echo e($r['text']); ?>"</p>
            <div class="flex items-center gap-2 pt-3 border-t border-white/10">
              <div class="w-7 h-7 rounded-full bg-gradient-to-br from-indigo-400 to-pink-400 flex items-center justify-center text-white text-xs">
                <i class="bi <?php echo e($r['icon']); ?>"></i>
              </div>
              <span class="text-[11px] font-semibold text-white/80"><?php echo e($r['name']); ?></span>
            </div>
          </div>
        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
      </div>
  ...
  <div class="max-w-3xl mx-auto space-y-2.5">
    <?php
      $faqs = [
        ...
        ['q' => 'ช่างภาพได้เงินเร็วแค่ไหน?', 'a' => 'ขั้นต่ำ ฿500 → โอนเข้าบัญชีไทยอัตโนมัติทุกวันจันทร์ผ่าน Omise · ไม่มีค่าธรรมเนียมแอบแฝง · 0% commission'],
    ...
    <?php $__currentLoopData = $faqs; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $i => $faq): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
      <div class="rounded-2xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-white/10 overflow-hidden p-anim d<?php echo e(min($i+2, 5)); ?>">
        <button type="button"
                @click="open = (open === <?php echo e($i+1); ?>) ? 0 : <?php echo e($i+1); ?>"
          ...
          <span class="font-semibold text-sm md:text-base text-slate-900 dark:text-white pr-2 flex items-center gap-2.5">
            <span class="text-indigo-500 dark:text-indigo-400 font-mono text-xs">0<?php echo e($i+1); ?></span>
            <?php echo e($faq['q']); ?>

          </span>
          <i class="bi bi-plus-lg text-slate-400 transition-transform shrink-0 text-lg"
             :class="open === <?php echo e($i+1); ?> ? 'rotate-45' : ''"></i>
        </button>
        <div x-show="open === <?php echo e($i+1); ?>" x-collapse>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\photo-gallery-pgsql\resources\views/public/promo.blade.php ENDPATH**/ ?>