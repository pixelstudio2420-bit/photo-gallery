<?php $__env->startSection('title', 'เริ่มขายรูปออนไลน์ — 0% commission · ส่งเข้า LINE อัตโนมัติ'); ?>
      ...
      <div class="relative">
        <div class="rounded-2xl bg-white dark:bg-slate-900/80 border border-slate-200 dark:border-white/10 p-6 h-full shadow-lg shadow-indigo-500/5">
          <div class="flex items-center gap-3 mb-4">
            ...
            <i class="bi <?php echo e($step['icon']); ?> text-2xl text-indigo-500 dark:text-indigo-300"></i>
          </div>
          <h3 class="font-bold text-slate-900 dark:text-white mb-2 text-base"><?php echo e($step['title']); ?></h3>
          <p class="text-sm text-slate-600 dark:text-slate-400 leading-relaxed"><?php echo e($step['body']); ?></p>
        </div>
        <?php if (! ($loop->last)): ?>
  ...
  <div class="max-w-7xl mx-auto px-4 sm:px-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4">
      <?php $__currentLoopData = $plans; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $plan): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
        ...
        ?>
        <div class="fp-card relative rounded-2xl p-5 flex flex-col <?php echo e($isFeatured ? 'bg-gradient-to-br from-indigo-600 to-violet-600 text-white border-2 border-indigo-400 shadow-2xl shadow-indigo-500/30 lg:scale-105 lg:z-10' : 'bg-white dark:bg-slate-900/80 border border-slate-200 dark:border-white/10 shadow-lg'); ?>">
          ...
          <span class="absolute -top-3 left-1/2 -translate-x-1/2 inline-flex items-center gap-1 px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wide bg-amber-400 text-amber-900 shadow-md">
            <i class="bi bi-star-fill"></i> <?php echo e($plan->badge); ?>

          </span>
          <?php endif; ?>

          <h3 class="font-bold text-base <?php echo e($isFeatured ? 'text-white' : 'text-slate-900 dark:text-white'); ?>">
            <?php echo e($plan->name); ?>

          </h3>
          <?php if($plan->tagline): ?>
          <p class="text-xs mt-1 mb-3 <?php echo e($isFeatured ? 'text-indigo-100' : 'text-slate-500 dark:text-slate-400'); ?>">
            <?php echo e($plan->tagline); ?>

          </p>
          <?php endif; ?>

          <div class="mt-3 mb-4">
            <span class="text-3xl font-extrabold <?php echo e($isFeatured ? 'text-white' : 'text-slate-900 dark:text-white'); ?>">
              <?php echo e($monthlyPriceText); ?>

            </span>
            <?php if((float) $plan->price_thb > 0): ?>
            <span class="text-xs <?php echo e($isFeatured ? 'text-indigo-100' : 'text-slate-500 dark:text-slate-400'); ?>">/ เดือน</span>
            <?php endif; ?>
          </div>

          <ul class="space-y-1.5 text-xs <?php echo e($isFeatured ? 'text-indigo-50' : 'text-slate-600 dark:text-slate-400'); ?> mb-5 flex-1">
            <?php $__currentLoopData = array_slice($features, 0, 6); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $feat): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
              <li class="flex items-start gap-1.5">
                <i class="bi bi-check2 <?php echo e($isFeatured ? 'text-emerald-300' : 'text-emerald-500'); ?> mt-0.5 flex-shrink-0"></i>
                <span><?php echo e($feat); ?></span>
              </li>
            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
            <?php if(count($features) > 6): ?>
              <li class="text-[11px] <?php echo e($isFeatured ? 'text-indigo-200' : 'text-slate-400 dark:text-slate-500'); ?> pl-5">
                + อีก <?php echo e(count($features) - 6); ?> ฟีเจอร์
              </li>
            <?php endif; ?>
          </ul>

          <a href="<?php echo e(route('photographer-onboarding.quick')); ?>?plan=<?php echo e($plan->code); ?>"
             class="inline-flex items-center justify-center gap-1.5 w-full px-4 py-2.5 rounded-xl font-semibold text-sm transition <?php echo e($isFeatured ? 'bg-white text-indigo-700 hover:bg-indigo-50 shadow-lg' : 'bg-indigo-600 text-white hover:bg-indigo-700 dark:bg-indigo-500 dark:hover:bg-indigo-400'); ?>">
            <?php if((float) $plan->price_thb === 0.0): ?>
              เริ่มฟรี
            <?php else: ?>
              เลือกแพ็กเกจนี้
            <?php endif; ?>
    ...
    <div class="space-y-3" x-data="{ open: 0 }">
      ...
      <div class="rounded-2xl bg-white dark:bg-slate-900/80 border border-slate-200 dark:border-white/10 overflow-hidden shadow-sm">
        <button type="button"
          ...
          <span class="font-semibold text-slate-900 dark:text-white text-base"><?php echo e($faq['q']); ?></span>
          <i class="bi text-indigo-500" :class="open === <?php echo e($i); ?> ? 'bi-dash-circle' : 'bi-plus-circle'"></i>
        </button>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\photo-gallery-pgsql\resources\views/public/for-photographers.blade.php ENDPATH**/ ?>