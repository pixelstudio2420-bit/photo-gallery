<?php $__env->startSection('title', 'Watermark Settings'); ?>

<?php $__env->startPush('styles'); ?>
<?php echo $__env->make('admin.settings._shared-styles', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
<style>
  /* Type radio card */
  .wm-type-card {
    display: block;
...
<div class="max-w-7xl mx-auto pb-16">
  ...
  <form method="POST" action="<?php echo e(route('admin.settings.watermark.update')); ?>" enctype="multipart/form-data">
    <?php echo csrf_field(); ?>
      ...
      <div class="p-5 flex items-center justify-between gap-4">
        ...
        <label class="tw-switch">
          <input type="checkbox" id="watermark_enabled" name="watermark_enabled" value="1"
                 <?php echo e(($settings['watermark_enabled'] ?? '0') == '1' ? 'checked' : ''); ?>>
      ...
      <div class="p-5">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-3 mb-5">
          <label class="block relative">
            <input type="radio" name="watermark_type" id="type_text" value="text" class="wm-type-radio sr-only"
                   <?php echo e(($settings['watermark_type'] ?? 'text') === 'text' ? 'checked' : ''); ?>
          ...
          <label class="block relative">
            <input type="radio" name="watermark_type" id="type_image" value="image" class="wm-type-radio sr-only"
                   <?php echo e(($settings['watermark_type'] ?? '') === 'image' ? 'checked' : ''); ?>
          ...
          <div>
            ...
            <div class="flex items-center gap-2">
              <input type="color" name="watermark_color"
                     value="<?php echo e($settings['watermark_color'] ?: '#ffffff'); ?>"
        ...
        <div id="section-image" class="pt-4 border-t border-slate-200 dark:border-white/10" style="display:none;">
          ...
          <?php if(!empty($settings['watermark_image_path'])): ?>
            <div class="mb-3">
              <img src="<?php echo e(Storage::url($settings['watermark_image_path'])); ?>"
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
          <div>
            ...
            <select name="watermark_position"
                    class="w-full px-3 py-2 rounded-lg text-sm
                           ...
                           text-slate-900 dark:text-slate-100
                           focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none">
              <?php $__currentLoopData = ['diagonal' => 'Diagonal (Across image)', 'tiled' => 'Tiled (Repeat)', 'center' => 'Center', 'bottom-right' => 'Bottom Right', 'bottom-left' => 'Bottom Left', 'top-right' => 'Top Right']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $val => $label): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <option value="<?php echo e($val); ?>" <?php echo e(($settings['watermark_position'] ?? 'diagonal') === $val ? 'selected' : ''); ?>><?php echo e($label); ?></option>
          ...
          <div>
            <label class="text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1.5 flex items-center justify-between">
              <span>Opacity</span>
              <span class="text-indigo-600 dark:text-indigo-400 font-bold" id="opacityVal"><?php echo e($settings['watermark_opacity'] ?: 50); ?>%</span>
            </label>
            <input type="range" name="watermark_opacity" class="tw-range" min="5" max="100" step="5"
                   value="<?php echo e($settings['watermark_opacity'] ?: 50); ?>"
          ...
          <div>
            <label class="text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1.5 flex items-center justify-between">
              <span>Size</span>
              <span class="text-indigo-600 dark:text-indigo-400 font-bold" id="sizeVal"><?php echo e($settings['watermark_size_percent'] ?: 30); ?>%</span>
            </label>
            <input type="range" name="watermark_size_percent" class="tw-range" min="10" max="80" step="5"
                   value="<?php echo e($settings['watermark_size_percent'] ?: 30); ?>"
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\photo-gallery-pgsql\resources\views/admin/settings/watermark.blade.php ENDPATH**/ ?>