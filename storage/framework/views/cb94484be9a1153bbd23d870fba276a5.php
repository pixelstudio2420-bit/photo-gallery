...
<?php if (! $__env->hasRenderedOnce('129076db-8230-46f0-9a1d-4502177c151d')): $__env->markAsRenderedOnce('129076db-8230-46f0-9a1d-4502177c151d'); ?>
<style>
            ...
            <div class="flex items-center justify-between gap-3 relative" style="z-index:2;">
                <div class="flex items-center gap-3 min-w-0">
                    ...
                    <div class="w-12 h-12 rounded-xl flex items-center justify-center text-white text-xl shrink-0"
                        ...
                        <i class="bi <?php echo e($plan?->iconClass() ?? 'bi-camera'); ?>"></i>
                    </div>
                    <div class="min-w-0">
                        <p class="text-[10px] font-bold tracking-[0.16em] uppercase opacity-80 m-0">แผนสมัครสมาชิก</p>
                        <h3 class="text-xl font-bold m-0 truncate" style="letter-spacing:-0.02em;"><?php echo e($plan?->name ?? 'ฟรี'); ?></h3>
                        <?php if($plan && !$isFree): ?>
                            <p class="text-xs opacity-90 m-0 mt-0.5">฿<?php echo e(number_format((float) $plan->price_thb, 0)); ?>/เดือน</p>
                        <?php endif; ?>
                    </div>
                </div>
        ...
        <div class="plan-card-body">
                ...
                <div class="flex items-baseline justify-between mb-2">
                    ...
                    <div class="flex items-baseline gap-1.5">
                        <span class="text-base font-bold text-gray-900 dark:text-white" style="font-variant-numeric:tabular-nums;">
                            <?php echo e(number_format($usedGb, 2)); ?>

                        </span>
                        <span class="text-xs text-gray-500 dark:text-gray-400 font-medium">/ <?php echo e(number_format($quotaGb, 0)); ?> GB</span>
                        <span class="text-xs font-semibold ml-2" style="color:<?php echo e($barColor); ?>;"><?php echo e(number_format($pct, 1)); ?>%</span>
                    </div>
                </div>
                <div class="plan-meter">
                    <div style="width:<?php echo e(min(100, $pct)); ?>%;background:linear-gradient(90deg,<?php echo e($barColor); ?> 0%,<?php echo e($barColor); ?>cc 100%);"></div>
                </div>
                <?php if($critical): ?>
                    <p class="text-[11px] mt-2 mb-0 text-rose-600 dark:text-rose-400">
                        <i class="bi bi-exclamation-triangle-fill"></i>
                        เต็มเกือบหมดแล้ว — ลบรูปเก่าหรือ
                        <a href="<?php echo e(route('photographer.subscription.plans')); ?>" class="font-semibold underline">อัปเกรดแผน</a>
                    </p>
                <?php elseif($warn): ?>
                    <p class="text-[11px] mt-2 mb-0 text-amber-600 dark:text-amber-400">
                        <i class="bi bi-info-circle-fill"></i> ใช้พื้นที่ไป <?php echo e(number_format($pct, 0)); ?>% — เริ่มใกล้เต็ม
                    </p>
                <?php endif; ?>
            </div>
            ...
            <div class="grid grid-cols-3 gap-2 mb-4">
                <div class="plan-stat text-center">
                    ...
                    <p class="font-bold text-sm text-gray-900 dark:text-white m-0" style="font-variant-numeric:tabular-nums;">
                        <?php echo e($eventsUsed); ?><span class="text-xs text-gray-400 font-medium">/<?php echo e($eventsUnlimited ? '∞' : $eventsCap); ?></span>
                    </p>
                </div>
                <div class="plan-stat text-center">
                    <p class="text-[10px] font-bold tracking-wider uppercase text-gray-500 dark:text-gray-400 m-0 mb-1">
                    ...
                    <p class="font-bold text-sm text-gray-900 dark:text-white m-0" style="font-variant-numeric:tabular-nums;">
                        <?php echo e(number_format($aiCreditsUsed)); ?><span class="text-xs text-gray-400 font-medium">/<?php echo e(number_format($aiCreditsCap)); ?></span>
                    </p>
                </div>
                <div class="plan-stat text-center">
                    <p class="text-[10px] font-bold tracking-wider uppercase text-gray-500 dark:text-gray-400 m-0 mb-1">
                    ...
                    <p class="font-bold text-sm text-gray-900 dark:text-white m-0">
                        <?php echo e(rtrim(rtrim(number_format($sharePct, 1), '0'), '.')); ?><span class="text-xs text-gray-400 font-medium">%</span>
                    </p>
                </div>
            </div>

            
            <?php if(count($aiFeatures) > 0): ?>
                <div class="mb-4">
                    <p class="text-[10px] font-bold tracking-[0.16em] uppercase text-gray-500 dark:text-gray-400 mb-2">
                        <i class="bi bi-magic mr-1"></i>ฟีเจอร์ที่ปลดล็อก (<?php echo e(count($aiFeatures)); ?>)
                    </p>
                    <div class="flex flex-wrap gap-1.5">
                        <?php $__currentLoopData = $aiFeatures; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $featureKey): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <?php $f = $featureLabels[$featureKey] ?? null; ?>
                            <?php if($f): ?>
                                <span class="feature-pill"><i class="bi <?php echo e($f[0]); ?>"></i><?php echo e($f[1]); ?></span>
                            <?php endif; ?>
            ...
            <div class="flex items-center justify-between gap-3 pt-3 border-t border-gray-100 dark:border-white/[0.06]">
                <div class="text-[11px] text-gray-500 dark:text-gray-400">
                    <?php if($renewAt && !$isFree): ?>
                        <i class="bi bi-arrow-clockwise"></i>
                        <?php if($cancelAtEnd): ?>
                            สิ้นสุด: <strong class="text-gray-700 dark:text-gray-200"><?php echo e(\Illuminate\Support\Carbon::parse($renewAt)->format('d M Y')); ?></strong>
                        <?php else: ?>
                            ต่ออายุ: <strong class="text-gray-700 dark:text-gray-200"><?php echo e(\Illuminate\Support\Carbon::parse($renewAt)->format('d M Y')); ?></strong>
                            <?php if($daysLeft !== null): ?>
                                <span class="text-gray-400">(อีก <?php echo e($daysLeft); ?> วัน)</span>
                            <?php endif; ?>
                        <?php endif; ?>
                    <?php else: ?>
                ...
                <?php if($isFree): ?>
                    <a href="<?php echo e(route('photographer.subscription.plans')); ?>" class="plan-cta no-underline text-xs">
                        <i class="bi bi-rocket-takeoff"></i> อัปเกรด
                    </a>
                <?php else: ?>
                    <a href="<?php echo e(route('photographer.subscription.plans')); ?>" class="text-[11px] font-semibold text-indigo-600 dark:text-indigo-400 hover:text-indigo-700 no-underline">
...
<?php endif; ?>
<?php /**PATH C:\xampp\htdocs\photo-gallery-pgsql\resources\views/photographer/partials/subscription-widget.blade.php ENDPATH**/ ?>