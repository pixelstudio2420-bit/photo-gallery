  ...
  <div class="stat-card pg-card">
    <div class="p-4 flex items-center gap-3">
      ...
      <div>
        ...
        <div class="font-bold text-lg text-indigo-600"><?php echo e(number_format($totalEarnings, 0)); ?></div>
      </div>
    </div>
  </div>
  <div class="stat-card pg-card">
    <div class="p-4 flex items-center gap-3">
      ...
      <div>
        ...
        <div class="font-bold text-lg text-emerald-500"><?php echo e(number_format($thisMonthEarnings, 0)); ?></div>
      </div>
    </div>
  </div>
  <div class="stat-card pg-card">
    <div class="p-4 flex items-center gap-3">
      ...
      <div>
        ...
        <div class="font-bold text-lg text-amber-500"><?php echo e(number_format($totalOrders)); ?></div>
      </div>
    </div>
  </div>
  <div class="stat-card pg-card">
    <div class="p-4 flex items-center gap-3">
      ...
      <div>
        ...
        <div class="font-bold text-lg text-violet-500"><?php echo e(number_format($totalPhotos)); ?> <span class="text-gray-500 font-normal text-xs">/ <?php echo e(number_format($totalViews)); ?> views</span></div>
      </div>
    </div>
  </div>
</div>
  ...
  <div class="lg:col-span-5 pg-card">
    <div class="p-5">
      ...
      <div class="top-event-row">
        <div class="rank-badge" style="background:<?php echo e($i < 3 ? 'rgba(245,158,11,0.1)' : '#f1f5f9'); ?>;color:<?php echo e($i < 3 ? '#f59e0b' : '#64748b'); ?>;"><?php echo e($i + 1); ?></div>
        <div class="flex-1 min-w-0">
          <div class="font-semibold text-sm truncate"><?php echo e($ev->name); ?></div>
          <div class="text-gray-500 text-xs"><?php echo e($ev->order_count); ?> orders</div>
        </div>
        <div class="font-bold text-sm text-indigo-600"><?php echo e(number_format($ev->revenue, 0)); ?></div>
      </div>
...
<?php if($hasCustomerAnalytics ?? false): ?>
<div class="grid grid-cols-1 lg:grid-cols-12 gap-4 mt-4">
    ...
    <?php else: ?>
      <div class="pg-table-wrap" style="border:none;border-radius:0;box-shadow:none;">
        <table class="pg-table pg-table--compact">
          ...
          <tbody>
            <?php $__currentLoopData = $topCustomers; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $c): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
              <tr>
                <td>
                  <p class="font-semibold m-0"><?php echo e($c->name ?? '—'); ?></p>
                  <p class="text-xs text-gray-500 m-0"><?php echo e($c->email ?? ''); ?></p>
                </td>
                <td class="text-end is-mono"><?php echo e($c->orders); ?></td>
                <td class="text-end is-mono font-bold text-violet-600"><?php echo e(number_format($c->spend, 0)); ?> ฿</td>
              </tr>
  ...
  <div class="lg:col-span-5 pg-card pg-card-padded pg-anim d3">
    ...
    <div class="text-center py-3">
      <p class="text-3xl font-bold text-emerald-600"><?php echo e(number_format($conversionRate, 2)); ?>%</p>
      <p class="text-xs text-gray-500 mt-1"><?php echo e(number_format($totalOrders)); ?> orders / <?php echo e(number_format($totalViews)); ?> views</p>
    </div>
    <hr class="my-4 border-gray-100">
    <p class="text-xs uppercase tracking-wider text-gray-500 font-medium mb-3">ช่วงเวลาที่ขายดี (60 วันล่าสุด)</p>
    <?php if(!empty($hourlyHeatmap) && count($hourlyHeatmap) > 0): ?>
      <div class="grid grid-cols-12 gap-0.5">
        <?php $maxHour = $hourlyHeatmap->max('orders') ?: 1; ?>
        <?php for($h = 0; $h < 24; $h++): ?>
          <?php
          ...
          <div class="aspect-square rounded text-[9px] flex items-center justify-center font-medium
                      <?php if($cnt > 0): ?> text-white <?php else: ?> text-gray-400 <?php endif; ?>"
               style="background-color: rgba(139, 92, 246, <?php echo e($opacity / 100); ?>); <?php if($opacity < 30 && $cnt > 0): ?> color: #6b21a8; <?php endif; ?>"
               title="<?php echo e($h); ?>:00 — <?php echo e($cnt); ?> orders">
            <?php echo e($h); ?>
...
<div class="mt-4 rounded-xl border border-violet-200 bg-violet-50 p-5">
  ...
  <p class="text-sm text-violet-800 mt-2">
    ...
    <a href="<?php echo e(route('photographer.subscription.plans')); ?>" class="font-medium underline">อัปเกรดเป็น Business เพื่อปลดล็อก</a>
  </p>
</div>
<?php endif; ?>
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (dailyCtx) {
    new Chart(dailyCtx, {
      type: 'bar',
      data: {
        labels: <?php echo json_encode($revenueDays->pluck('label')); ?>,
        datasets: [{
          label: 'รายได้ (THB)',
          data: <?php echo json_encode($revenueDays->pluck('total')); ?>,
  ...
  if (monthlyCtx) {
    new Chart(monthlyCtx, {
      type: 'line',
      data: {
        labels: <?php echo json_encode($revenueMonths->pluck('label')); ?>,
        datasets: [
          {
            label: 'รายได้สุทธิ',
            data: <?php echo json_encode($revenueMonths->pluck('total')); ?>,
            borderColor: '#2563eb',
          ...
          {
            label: 'ยอดขายรวม',
            data: <?php echo json_encode($revenueMonths->pluck('gross')); ?>,
            borderColor: '#10b981',
  ...
  if (statusCtx) {
    const statusData = <?php echo json_encode($orderStatuses); ?>;
...
<?php echo $__env->make('layouts.photographer', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\photo-gallery-pgsql\resources\views/photographer/analytics/index.blade.php ENDPATH**/ ?>