<?php $__env->startSection('title', ($item->exists ? 'แก้ไข' : 'เพิ่ม') . ' Addon · Monetization'); ?>
...
<div class="max-w-7xl mx-auto p-4 md:p-6">
        ...
        <div class="flex flex-wrap items-start justify-between gap-4">
            <div class="flex items-start gap-4 min-w-0">
                ...
                <div class="shrink-0 w-14 h-14 rounded-2xl flex items-center justify-center text-2xl text-white shadow-lg"
                     style="background:<?php echo e($shape['accent']); ?>;">
                    <i class="bi <?php echo e($shape['icon']); ?>"></i>
                </div>
                <div class="min-w-0">
                    <h1 class="text-2xl md:text-3xl font-extrabold text-slate-900 dark:text-white tracking-tight">
                        <?php echo e($item->exists ? $item->label : 'เพิ่ม Addon ใหม่'); ?>
                    ...
                    <div class="flex flex-wrap items-center gap-2 mt-1.5 text-xs">
                        ...
                        <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full font-semibold ring-1 ring-inset"
                              style="background:<?php echo e($toneBg[$shape['tone']] ?? $toneBg['indigo']); ?>;
                                     color:<?php echo e($shape['accent']); ?>;">
                            <i class="bi <?php echo e($shape['icon']); ?>"></i> <?php echo e($shape['title']); ?>

                        </span>
                        <?php if($item->exists): ?>
                            <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full font-semibold ring-1 ring-inset <?php echo e($item->is_active ? 'text-emerald-700 dark:text-emerald-300 ring-emerald-300/50' : 'text-slate-700 dark:text-slate-300 ring-slate-300/50'); ?>"
                                  style="background:<?php echo e($item->is_active ? $toneBg['emerald'] : $toneBg['slate']); ?>;">
                                <?php if($item->is_active): ?> ✓ เปิดขาย <?php else: ?> ✗ ปิดอยู่ <?php endif; ?>
                            </span>
                            <?php if($item->trashed()): ?>
                                <span class="inline-flex items-center px-2 py-0.5 rounded-full font-semibold ring-1 ring-rose-300/60 text-rose-700 dark:text-rose-300"
                                      style="background:<?php echo e($toneBg['rose']); ?>;">soft-deleted</span>
                            <?php endif; ?>
                        <?php endif; ?>
                    </div>
                </div>
    ...
    <?php if($errors->any()): ?>
        <div class="mb-4 rounded-xl ring-1 ring-rose-300/60 px-4 py-3 text-sm"
             style="background:<?php echo e($toneBg['rose']); ?>;">
    ...
    <?php if(session('success')): ?>
        <div class="mb-4 rounded-xl ring-1 ring-emerald-300/60 px-4 py-3 text-sm text-emerald-700 dark:text-emerald-300"
             style="background:<?php echo e($toneBg['emerald']); ?>;">
    ...
    <form method="POST" id="addonForm"
                ...
                <section class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm overflow-hidden">
                    ...
                    <div class="p-6 grid md:grid-cols-2 gap-5">
                        ...
                        <div class="md:col-span-1">
                            <label class="<?php echo e($cls['label']); ?>">
                                ...
                                <?php if($item->exists): ?>
                                    <span class="text-[10px] uppercase font-bold text-amber-700 dark:text-amber-300 px-1.5 py-0.5 rounded ring-1 ring-amber-300/60"
                                          style="background:<?php echo e($toneBg['amber']); ?>;">
                                        🔒 immutable
                                    </span>
                                <?php endif; ?>
                            </label>
                            <input type="text" name="sku" required pattern="[a-z0-9_\.]+" maxlength="60"
                                   ...
                                   <?php echo e($item->exists ? 'readonly' : ''); ?>

                                   placeholder="เช่น storage.50gb"
                                   class="<?php echo e($cls['input']); ?> font-mono <?php echo e($item->exists ? $cls['locked'] : ''); ?>">
                            <p class="<?php echo e($cls['help']); ?>">
                                <?php if($item->exists): ?>
                        ...
                        <div class="md:col-span-1">
                            ...
                            <select name="category" id="category" required
                                    <?php echo e($item->exists ? 'disabled' : ''); ?>

                                    class="<?php echo e($cls['select']); ?> <?php echo e($item->exists ? $cls['locked'] : ''); ?>">
                                <?php $__currentLoopData = $categories; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $code => $label): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <option value="<?php echo e($code); ?>" <?php if(old('category', $item->category)===$code): echo 'selected'; endif; ?>><?php echo e($label); ?></option>
                        ...
                        <div class="md:col-span-2">
                            <label class="<?php echo e($cls['label']); ?>">
                                ...
                                <span class="text-xs text-slate-400 font-normal" id="labelCount"><?php echo e(mb_strlen(old('label', $item->label ?? ''))); ?>/120</span>
                            </label>
                            <input type="text" name="label" required maxlength="120"
                                   value="<?php echo e(old('label', $item->label)); ?>"
                                   placeholder="เช่น +50 GB"
                                   class="<?php echo e($cls['input']); ?> font-semibold !text-base"
                            ...
                            <p class="<?php echo e($cls['help']); ?>">หัวเรื่องบนการ์ดในร้าน</p>
                        </div>
                        ...
                        <div>
                            ...
                            <div class="relative">
                                ...
                                <input type="number" name="price_thb" required min="0" max="999999" step="1"
                                       value="<?php echo e(old('price_thb', $item->price_thb)); ?>"
                                       class="<?php echo e($cls['input']); ?> !pl-9 !text-base !font-bold !text-indigo-700 dark:!text-indigo-300"
                                       oninput="window.dispatchEvent(new CustomEvent('addon-preview-update', { detail: { field: 'price_thb', value: this.value } }))">
                            </div>
                            <p class="<?php echo e($cls['help']); ?>">ราคาที่ช่างภาพจ่ายผ่าน checkout</p>
                        </div>
                        ...
                        <div>
                            ...
                            <input type="number" name="sort_order" min="0" max="9999"
                                   ...
                                   class="<?php echo e($cls['input']); ?>">
                            <p class="<?php echo e($cls['help']); ?>">เลขน้อย = แสดงก่อน · ปกติเริ่ม 10, 20, 30…</p>
                        </div>
                        ...
                        <div class="md:col-span-2">
                            <label class="<?php echo e($cls['label']); ?>">
                                ...
                                <span class="text-xs text-slate-400 font-normal" id="taglineCount"><?php echo e(mb_strlen(old('tagline', $item->tagline ?? ''))); ?>/200</span>
                            </label>
                            <input type="text" name="tagline" maxlength="200"
                                   value="<?php echo e(old('tagline', $item->tagline)); ?>"
                            ...
                            <p class="<?php echo e($cls['help']); ?>">ข้อความเล็กใต้ชื่อรายการ ใช้บอกประโยชน์/use-case</p>
                        </div>
                        ...
                        <div>
                            ...
                            <p class="<?php echo e($cls['help']); ?>">ป้ายเล็กบนการ์ด — แสดงสีเหลือง</p>
                        </div>
                ...
                <section class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm overflow-hidden">
                    ...
                    <div class="p-6">
                        ...
                        <div data-cat="promotion" class="cat-block grid md:grid-cols-3 gap-5">
                            <div>
                                <label class="<?php echo e($cls['label']); ?>"><span>Kind</span></label>
                                <select name="meta_kind" class="<?php echo e($cls['select']); ?>">
                                    <?php $__currentLoopData = ['boost'=>'Boost — เพิ่มอันดับค้นหา','featured'=>'Featured — การ์ดเด่น','highlight'=>'Highlight — กรอบ + ป้าย']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $v=>$l): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                        <option value="<?php echo e($v); ?>" <?php if(old('meta_kind', $meta['kind'] ?? 'boost')===$v): echo 'selected'; endif; ?>><?php echo e($l); ?></option>
                                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                                </select>
                            </div>
                            <div>
                                <label class="<?php echo e($cls['label']); ?>"><span>รอบบิล</span></label>
                                <select name="meta_cycle" class="<?php echo e($cls['select']); ?>">
                                    <?php $__currentLoopData = ['daily'=>'รายวัน','monthly'=>'รายเดือน','yearly'=>'รายปี']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $v=>$l): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                        <option value="<?php echo e($v); ?>" <?php if(old('meta_cycle', $meta['cycle'] ?? 'monthly')===$v): echo 'selected'; endif; ?>><?php echo e($l); ?></option>
                                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                                </select>
                            </div>
                            <div>
                                <label class="<?php echo e($cls['label']); ?>"><span>Boost Score (1-25)</span></label>
                                <input type="number" name="meta_boost_score" min="1" max="25"
                                       value="<?php echo e(old('meta_boost_score', $meta['boost_score'] ?? 10)); ?>"
                                       class="<?php echo e($cls['input']); ?> !font-bold">
                                <p class="<?php echo e($cls['help']); ?>">↑ มาก = ขึ้นอันดับสูงกว่าในผลค้นหา</p>
                            </div>
                        </div>

                        
                        <div data-cat="storage" class="cat-block">
                            <label class="<?php echo e($cls['label']); ?>"><span>พื้นที่เพิ่ม</span></label>
                            <div class="flex items-stretch gap-2">
                                <input type="number" name="meta_storage_gb" min="1" max="10240"
                                       value="<?php echo e(old('meta_storage_gb', $meta['storage_gb'] ?? 50)); ?>"
                                       class="<?php echo e($cls['input']); ?> max-w-[10rem] !font-bold">
                            ...
                            <p class="<?php echo e($cls['help']); ?>">เพิ่มจาก quota แผน subscription ปัจจุบันของช่างภาพ</p>
                        </div>

                        
                        <div data-cat="ai_credits" class="cat-block">
                            <label class="<?php echo e($cls['label']); ?>"><span>จำนวน Credits</span></label>
                            <div class="flex items-stretch gap-2">
                                <input type="number" name="meta_credits" min="100" max="1000000" step="100"
                                       value="<?php echo e(old('meta_credits', $meta['credits'] ?? 5000)); ?>"
                                       class="<?php echo e($cls['input']); ?> max-w-[12rem] !font-bold">
                                <div class="h-11 px-4 flex items-center rounded-xl bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 font-semibold text-sm">
                                    credits
                                </div>
                            </div>
                            <p class="<?php echo e($cls['help']); ?>">ใช้ใน AI Face Search · Best Shot · Auto-tag — 1 credit = 1 ภาพ</p>
                        </div>
                        ...
                        <div data-cat="branding" class="cat-block space-y-5">
                            <label class="flex items-center gap-3 cursor-pointer px-3.5 py-3 rounded-xl ring-1 ring-slate-200 dark:ring-slate-600 hover:ring-indigo-300 dark:hover:ring-indigo-500 transition has-[:checked]:ring-2 has-[:checked]:ring-indigo-500/40 has-[:checked]:bg-indigo-50/50 dark:has-[:checked]:bg-indigo-900/20">
                                <input type="checkbox" name="meta_one_time" value="1"
                                       <?php if(old('meta_one_time', !empty($meta['one_time']))): echo 'checked'; endif; ?>
                            ...
                            <div>
                                <label class="<?php echo e($cls['label']); ?>"><span>หรือกำหนด Cycle (ถ้าไม่ใช่ one-time)</span></label>
                                <select name="meta_cycle" class="<?php echo e($cls['select']); ?> max-w-[14rem]">
                                    <?php $__currentLoopData = ['monthly'=>'รายเดือน','yearly'=>'รายปี']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $v=>$l): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                        <option value="<?php echo e($v); ?>" <?php if(old('meta_cycle', $meta['cycle'] ?? 'monthly')===$v): echo 'selected'; endif; ?>><?php echo e($l); ?></option>
                                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                                </select>
                            </div>
                        ...
                        <div data-cat="priority" class="cat-block">
                            ...
                            <p class="<?php echo e($cls['help']); ?>">รอบเก็บค่าบริการสำหรับ Priority Lane</p>
                        </div>
                    </div>
                </section>

                
                <?php if($item->exists && $recentBuyers->count() > 0): ?>
                    <section class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm overflow-hidden">
                        <header class="px-6 py-4 border-b border-slate-100 dark:border-slate-700">
                            <h2 class="font-bold text-slate-900 dark:text-white flex items-center gap-2">
                                <i class="bi bi-people text-emerald-500"></i> ผู้ซื้อล่าสุด
                                <span class="text-sm font-normal text-slate-500 ml-1">(ล่าสุด <?php echo e($recentBuyers->count()); ?> รายการ)</span>
                            </h2>
                        </header>
                        <ul class="divide-y divide-slate-100 dark:divide-slate-700">
                            <?php $__currentLoopData = $recentBuyers; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $buyer): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <li class="px-6 py-3 flex items-center justify-between text-sm">
                                    <div class="min-w-0">
                                        <div class="font-semibold text-slate-900 dark:text-white truncate">
                                            <?php echo e($buyer->display_name ?? $buyer->first_name ?? $buyer->email ?? 'Unknown'); ?>

                                        </div>
                                        <div class="text-xs text-slate-500 truncate"><?php echo e($buyer->email); ?></div>
                                    </div>
                                    <div class="text-right shrink-0 ml-4">
                                        <div class="text-xs text-slate-500">
                                            <?php echo e(\Carbon\Carbon::parse($buyer->created_at)->diffForHumans()); ?>

                                        </div>
                                        <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold ring-1
                                            <?php echo e($buyer->status === 'activated' ? 'text-emerald-700 dark:text-emerald-300 ring-emerald-300/50' : 'text-amber-700 dark:text-amber-300 ring-amber-300/50'); ?>"
                                              style="background:<?php echo e($buyer->status === 'activated' ? $toneBg['emerald'] : $toneBg['amber']); ?>;">
                                            <?php echo e($buyer->status); ?>
                    ...
                    <section class="rounded-2xl ring-1 ring-rose-300/60 overflow-hidden"
                        ...
                        <header class="px-6 py-4 border-b border-rose-300/40">
                            ...
                            <p class="text-xs text-rose-700 dark:text-rose-300 mt-0.5">
                                <?php if($hasPurchases): ?>
                                    มีประวัติการซื้อ <?php echo e(number_format((int) $purchaseStats->total)); ?> รายการ — การลบจะเป็น soft-delete (รักษา audit) แต่ผู้ซื้อยังเห็นในประวัติของตัวเอง
                                <?php else: ?>
                        ...
                        <div class="p-6 flex flex-wrap items-center justify-between gap-3">
                            ...
                            <form method="POST" action="<?php echo e(route('admin.monetization.addons.destroy', $item->id)); ?>"
                                  onsubmit="return confirm('<?php echo e($hasPurchases ? "Soft-delete \"" . $item->label . "\" (มี " . $purchaseStats->total . " ประวัติการซื้อ) — ดำเนินการ?" : "ลบ \"" . $item->label . "\" ออกจากฐานข้อมูล?"); ?>');">
                                ...
                                <button class="inline-flex items-center gap-2 px-4 py-2 rounded-lg ring-1 ring-rose-400 text-rose-800 dark:text-rose-100 font-bold text-sm transition hover:bg-rose-100 dark:hover:bg-rose-900/40"
                                    ...
                                    <?php echo e($hasPurchases ? 'Soft Delete' : 'ลบทันที'); ?>
                        ...
                        <div id="addonPreviewCard"
                            ...
                            <div class="flex items-start justify-between mb-3">
                                <div class="w-10 h-10 rounded-xl flex items-center justify-center text-white"
                                     style="background:<?php echo e($shape['accent']); ?>;">
                                    <i class="bi <?php echo e($shape['icon']); ?> text-lg"></i>
                                </div>
                        ...
                        <dl class="space-y-2.5 text-sm">
                            <div class="flex justify-between items-baseline border-b border-slate-100 dark:border-slate-700 pb-2">
                                ...
                                <dd class="font-extrabold text-emerald-600 text-lg">
                                    <?php echo e($money($purchaseStats->gross_revenue ?? 0)); ?>

                                </dd>
                            </div>
                            <div class="flex justify-between">
                                <dt class="text-slate-500">ขายแล้ว</dt>
                                <dd class="font-bold"><?php echo e(number_format((int) $purchaseStats->total)); ?></dd>
                            </div>
                            <div class="flex justify-between">
                                <dt class="text-slate-500">เปิดใช้งาน</dt>
                                <dd class="text-emerald-600 font-semibold">
                                    <?php echo e(number_format((int) $purchaseStats->activated)); ?>
                            ...
                            <div class="flex justify-between">
                                ...
                                <dd class="text-amber-600 font-semibold">
                                    <?php echo e(number_format((int) $purchaseStats->pending)); ?>

                                </dd>
                            </div>
                            <div class="flex justify-between">
                                <dt class="text-slate-500">หมดอายุ</dt>
                                <dd class="text-slate-500"><?php echo e(number_format((int) $purchaseStats->expired)); ?></dd>
                            </div>
                        </dl>
                    </section>
                    ...
                    <section class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm p-5">
                        ...
                        <dl class="space-y-2 text-sm">
                            <div class="flex justify-between">
                                ...
                                <dd class="font-mono text-slate-700 dark:text-slate-300">#<?php echo e($item->id); ?></dd>
                            </div>
                            <div class="flex justify-between">
                                <dt class="text-slate-500">สร้างเมื่อ</dt>
                                <dd class="text-slate-700 dark:text-slate-300 text-xs"><?php echo e($item->created_at?->format('d/m/y H:i')); ?></dd>
                            </div>
                            <div class="flex justify-between">
                                <dt class="text-slate-500">แก้ไขล่าสุด</dt>
                                <dd class="text-slate-700 dark:text-slate-300 text-xs"><?php echo e($item->updated_at?->diffForHumans()); ?></dd>
                            </div>
                            <?php if($item->trashed()): ?>
                                <div class="flex justify-between pt-2 border-t border-slate-100 dark:border-slate-700">
                                    <dt class="text-rose-600">ลบเมื่อ</dt>
                                    <dd class="text-rose-600 text-xs"><?php echo e($item->deleted_at->format('d/m/y H:i')); ?></dd>
                                </div>
                            <?php endif; ?>
                        </dl>
                    </section>
        ...
        <div class="sticky bottom-4 mt-6 bg-white dark:bg-slate-800 rounded-2xl shadow-xl ring-1 ring-slate-200 dark:ring-slate-700 p-4 flex flex-wrap items-center justify-between gap-3">
            <div class="text-xs text-slate-500">
                <?php if($item->exists): ?>
                    แก้ไขรายการ <strong class="text-slate-700 dark:text-slate-300"><?php echo e($item->label); ?></strong>
                <?php else: ?>
                    เพิ่มรายการใหม่ในหมวด <strong class="text-slate-700 dark:text-slate-300" id="hintCategory"><?php echo e($shape['title']); ?></strong>
                <?php endif; ?>
            </div>
            <div class="flex gap-2 ml-auto">
                ...
                <button type="submit" class="inline-flex items-center gap-2 px-6 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white font-bold text-sm transition shadow-md">
                    ...
                    <?php echo e($item->exists ? 'บันทึกการเปลี่ยนแปลง' : 'สร้าง Addon'); ?>
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\photo-gallery-pgsql\resources\views/admin/monetization/addons/form.blade.php ENDPATH**/ ?>