<?php $__env->startSection('title', 'อัพโหลดรูปภาพ — ' . $event->name); ?>
...
<div class="mb-5">
  <nav class="flex items-center gap-2 text-xs text-gray-500 dark:text-gray-400 mb-2">
    <a href="<?php echo e(route('photographer.events.index')); ?>" class="hover:text-blue-600 dark:hover:text-blue-400">อีเวนต์</a>
    <i class="bi bi-chevron-right text-[10px]"></i>
    <a href="<?php echo e(route('photographer.events.show', $event)); ?>" class="hover:text-blue-600 dark:hover:text-blue-400 truncate max-w-[220px]"><?php echo e($event->name); ?></a>
    <i class="bi bi-chevron-right text-[10px]"></i>
    <a href="<?php echo e(route('photographer.events.photos.index', $event)); ?>" class="hover:text-blue-600 dark:hover:text-blue-400">จัดการรูป</a>
    <i class="bi bi-chevron-right text-[10px]"></i>
  ...
  <div class="flex flex-wrap items-center justify-between gap-3">
    <div class="flex items-center gap-3">
      ...
      <div>
        ...
        <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">เพิ่มรูปใหม่เข้าสู่อีเวนต์ <?php echo e($event->name); ?></p>
      </div>
    </div>
    <div class="flex items-center gap-2">
      <a href="<?php echo e(route('photographer.events.photos.index', $event)); ?>"
      ...
      <a href="<?php echo e(route('photographer.events.show', $event)); ?>"
...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.event-cover','data' => ['src' => $event->cover_image_url,'name' => $event->name,'eventId' => $event->id,'size' => 'thumb']] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php endif; ?>
<?php $component->withAttributes(['src' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute($event->cover_image_url),'name' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute($event->name),'event-id' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute($event->id),'size' => 'thumb']); ?>
...
<?php endif; ?>
    <div class="min-w-0 flex-1">
      <h2 class="font-semibold text-gray-900 dark:text-gray-100 truncate"><?php echo e($event->name); ?></h2>
      <div class="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-gray-500 dark:text-gray-400 mt-1">
        <span class="inline-flex items-center gap-1">
          <i class="bi bi-images"></i>
          <span class="font-medium text-gray-700 dark:text-gray-200"><?php echo e(number_format($photoCount)); ?></span> รูปภาพ
        </span>
...
window.__nsfwConfig = {
  enabled: true,
  hardBlock: <?php echo e($nsfwHardBlock); ?>,
  warn: <?php echo e($nsfwWarn); ?>,
  model: null,
  loading: null,
  loadError: null,
  async loadModel() {
...
window.__photoPerf = {
  clientCompress:     <?php echo json_encode($clientCompress, 15, 512) ?>,
  clientMaxDimension: <?php echo e(max(800, min(8000, $clientMaxDimension))); ?>,
  // Canvas.toBlob expects quality 0..1
  clientQuality:      <?php echo e(max(50, min(100, $clientQuality)) / 100); ?>,
};
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  const uploadUrl   = <?php echo json_encode(route('photographer.events.photos.store', $event), 512) ?>;
...
<?php echo $__env->make('layouts.photographer', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\photo-gallery-pgsql\resources\views/photographer/photos/upload.blade.php ENDPATH**/ ?>