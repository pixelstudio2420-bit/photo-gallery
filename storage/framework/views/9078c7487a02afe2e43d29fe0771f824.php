<?php $__env->startSection('title', $profile->display_name . ' · ช่างภาพมืออาชีพ'); ?>
...
<header class="relative isolate overflow-hidden"
        style="min-height:580px;
               <?php if($heroImage): ?> background:linear-gradient(0deg, rgba(15,23,42,0.85), rgba(15,23,42,0.55)), url('<?php echo e($heroImage); ?>') center/cover no-repeat;
               <?php else: ?>          background:linear-gradient(135deg,#0f172a 0%,#1e1b4b 50%,#581c87 100%);
               <?php endif; ?>">
        ...
        <div class="flex flex-col items-center text-center">
            ...
            <div class="relative mb-5">
                ...
                <div class="relative w-28 h-28 md:w-32 md:h-32 rounded-full overflow-hidden ring-4 ring-white/30 shadow-2xl bg-gradient-to-br from-indigo-500 to-violet-600 text-white text-4xl font-extrabold flex items-center justify-center">
                    <?php if($profile->avatar): ?>
                        <img src="<?php echo e(\Illuminate\Support\Facades\Storage::disk(config('media.disk', 'r2'))->url($profile->avatar)); ?>"
                             alt="" class="w-full h-full object-cover">
                    <?php else: ?>
                        <?php echo e(mb_strtoupper(mb_substr($displayName, 0, 1, 'UTF-8'), 'UTF-8')); ?>
            ...
            <div class="flex flex-wrap items-center justify-center gap-2 mb-3">
                ...
                <span class="inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider text-white shadow"
                      style="background:<?php echo e($tierBadgeColor); ?>;">
                    <?php echo e($isPro ? '⭐' : ''); ?> <?php echo e($tierBadgeLabel); ?>
            ...
            <div class="flex flex-wrap items-center justify-center gap-3 text-sm text-white/80 mb-5">
                <?php if($profile->photographer_code): ?>
                    <code class="px-2 py-0.5 rounded bg-white/15 backdrop-blur font-mono text-xs">
                        <?php echo e($profile->photographer_code); ?>

                    </code>
                <?php endif; ?>
                <?php if($profile->province): ?>
                    <span><i class="bi bi-geo-alt-fill"></i> <?php echo e($profile->province->name_th); ?></span>
                <?php endif; ?>
                <?php if($profile->years_experience): ?>
                    <span><i class="bi bi-clock-history"></i> ประสบการณ์ <?php echo e($profile->years_experience); ?> ปี</span>
                <?php endif; ?>
            </div>
            ...
            <?php if($profile->bio): ?>
                <p class="max-w-2xl text-white/90 text-base md:text-lg leading-relaxed mb-6">
                    <?php echo e(\Illuminate\Support\Str::limit($profile->bio, 180)); ?>

                </p>
            <?php endif; ?>

            
            <?php if(!empty($specialtyList)): ?>
                <div class="flex flex-wrap items-center justify-center gap-2 mb-7">
                    <?php $__currentLoopData = array_slice($specialtyList, 0, 6); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $sp): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold text-white backdrop-blur"
                              style="background:rgba(255,255,255,0.15);border:1px solid rgba(255,255,255,0.25);">
                            <?php echo e($specialtyLabels[$sp] ?? $sp); ?>
            ...
            <div class="flex flex-wrap items-center justify-center gap-4 md:gap-8 text-white/95 mb-8">
                <div class="text-center">
                    <div class="text-2xl md:text-3xl font-extrabold"><?php echo e(number_format($totalEvents)); ?></div>
                    <div class="text-xs uppercase tracking-wider opacity-75">อีเวนต์</div>
                </div>
                <div class="w-px h-10 bg-white/20"></div>
                <div class="text-center">
                    <div class="text-2xl md:text-3xl font-extrabold"><?php echo e(number_format($totalViews)); ?></div>
                ...
                <?php if($avgRating): ?>
                    ...
                    <div class="text-center">
                        ...
                        <div class="text-xs uppercase tracking-wider opacity-75"><?php echo e(number_format($totalReviewCount)); ?> รีวิว</div>
                    </div>
                <?php endif; ?>
            </div>
            ...
            <div class="flex flex-wrap items-center justify-center gap-3">
                ...
                <a href="<?php echo e(route('bookings.create', $profile->user_id)); ?>"
                ...
                <?php if($chatEnabled && auth()->check() && auth()->id() !== $profile->user_id): ?>
                    <form method="POST" action="<?php echo e(route('chat.start', $profile->user_id)); ?>" class="inline">
                        <?php echo csrf_field(); ?>
                        <button type="submit"
                                class="inline-flex items-center gap-2 px-6 py-3 rounded-xl font-semibold text-base text-white border border-white/30 backdrop-blur-md hover:bg-white/15 transition"
                                style="background:rgba(255,255,255,0.10);">
    ...
    <div class="grid md:grid-cols-3 gap-8">
        <div class="md:col-span-2">
            <h2 class="text-2xl md:text-3xl font-extrabold text-slate-900 dark:text-white tracking-tight mb-4">
                เกี่ยวกับ<?php echo e($displayName); ?>

            </h2>
            <?php if($profile->bio): ?>
                <p class="text-slate-600 dark:text-slate-300 text-base leading-relaxed mb-5 whitespace-pre-line"><?php echo e($profile->bio); ?></p>
            <?php else: ?>
            ...
            <?php if(!empty($specialtyList)): ?>
                ...
                <div class="flex flex-wrap gap-2">
                    <?php $__currentLoopData = $specialtyList; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $sp): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            ...
            <dl class="space-y-3 text-sm">
                <?php if($profile->years_experience): ?>
                    <div class="flex items-center justify-between">
                        ...
                        <dd class="font-semibold text-slate-900 dark:text-white"><?php echo e($profile->years_experience); ?> ปี</dd>
                    </div>
                <?php endif; ?>
                <div class="flex items-center justify-between">
                    <dt class="text-slate-500"><i class="bi bi-images mr-1.5"></i>อีเวนต์ทำเสร็จ</dt>
                    <dd class="font-semibold text-slate-900 dark:text-white"><?php echo e(number_format($totalEvents)); ?></dd>
                </div>
                <?php if($profile->province): ?>
                    <div class="flex items-center justify-between">
                        <dt class="text-slate-500"><i class="bi bi-geo-alt mr-1.5"></i>พื้นที่</dt>
                        <dd class="font-semibold text-slate-900 dark:text-white truncate ml-2 max-w-[10rem] text-right"><?php echo e($profile->province->name_th); ?></dd>
                    </div>
                <?php endif; ?>
                <?php if($avgRating): ?>
                    <div class="flex items-center justify-between">
                        ...
                        <dd class="font-semibold text-amber-600">
                            <i class="bi bi-star-fill mr-1"></i><?php echo e(number_format($avgRating, 1)); ?>/5
                        </dd>
                    </div>
                <?php endif; ?>
...
<section id="portfolio" class="max-w-6xl mx-auto px-4 md:px-6 py-12 scroll-mt-20">
    ...
    <?php if($galleryItems->count() > 0): ?>
        ...
        <div class="<?php echo e($gridClass); ?>">
            <?php $__currentLoopData = $galleryItems; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $i => $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <a href="<?php echo e(route('events.show', $item->slug ?? $item->id)); ?>"
                   class="group relative overflow-hidden rounded-2xl shadow-sm hover:shadow-2xl transition-all duration-300
                          <?php echo e(($useFeatured && $i === 0) ? 'col-span-2 row-span-2' : ''); ?>"
                   style="aspect-ratio: <?php echo e(($useFeatured && $i === 0) ? '4/3' : ($galleryItems->count() === 1 ? '16/9' : '1/1')); ?>;">
                    ...
                    <?php else: ?>
                        <div class="absolute inset-0 bg-gradient-to-br
                            <?php switch($i % 4):
                                case (0): ?> from-indigo-500 via-violet-500 to-pink-500 <?php break; ?>
                                <?php case (1): ?> from-emerald-500 via-teal-500 to-cyan-500 <?php break; ?>
                                <?php case (2): ?> from-amber-500 via-orange-500 to-rose-500 <?php break; ?>
                                <?php default: ?> from-purple-500 via-pink-500 to-rose-500
                            <?php endswitch; ?>
                        "></div>
                    <?php endif; ?>
                    ...
                    <div class="absolute inset-x-0 bottom-0 p-4 md:p-5 text-white">
                        ...
                        <h3 class="font-bold text-base md:text-lg leading-tight line-clamp-2 mb-1
                                   <?php echo e($i === 0 ? 'md:text-2xl' : ''); ?>">
                            <?php echo e($item->name); ?>

                        </h3>
                        <div class="flex items-center justify-between text-xs opacity-90">
                            <span>
                                <?php if($item->shoot_date): ?>
                                    <i class="bi bi-calendar3"></i>
                                    <?php echo e(\Carbon\Carbon::parse($item->shoot_date)->format('d M Y')); ?>
    ...
    <section id="events" class="max-w-6xl mx-auto px-4 md:px-6 py-12 scroll-mt-20">
        <div class="flex items-end justify-between flex-wrap gap-3 mb-6">
            ...
            <?php if($events->total() > $events->count()): ?>
                <div class="text-sm text-slate-500">
                    <?php echo e($events->total()); ?> อีเวนต์
                </div>
            <?php endif; ?>
        </div>
        ...
        <?php if($categories->count() > 0): ?>
            <div class="flex flex-wrap gap-2 mb-6">
                <a href="<?php echo e(route('photographers.show.slug', $profile->slug ?? '')); ?>"
                   class="px-3 py-1.5 rounded-full text-sm font-semibold transition
                          <?php echo e(!request('category') ? 'bg-indigo-600 text-white' : 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700'); ?>">
                    ทั้งหมด
                </a>
                <?php $__currentLoopData = $categories; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $cat): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <a href="?category=<?php echo e($cat->id); ?>"
                       class="px-3 py-1.5 rounded-full text-sm font-semibold transition
                              <?php echo e(request('category') == $cat->id ? 'bg-indigo-600 text-white' : 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700'); ?>">
                        <?php echo e($cat->name); ?>
        ...
        ?>
        <div class="<?php echo e($eventsGridClass); ?>">
            <?php $__currentLoopData = $events; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $ev): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <a href="<?php echo e(route('events.show', $ev->slug ?? $ev->id)); ?>"
                   class="block bg-white dark:bg-slate-800 rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 hover:-translate-y-1 ring-1 ring-slate-100 dark:ring-slate-700">
                    <div class="relative aspect-[4/3] overflow-hidden bg-gradient-to-br from-slate-200 to-slate-300 dark:from-slate-700 dark:to-slate-800">
                        <?php if($ev->cover_image_url): ?>
                            <img src="<?php echo e($ev->cover_image_url); ?>"
                        ...
                        <?php else: ?>
                            <div class="absolute inset-0 flex items-center justify-center text-white text-5xl font-extrabold opacity-30">
                                <?php echo e(mb_substr($ev->name, 0, 2, 'UTF-8')); ?>
                    ...
                    <div class="p-4">
                        <h3 class="font-bold text-slate-900 dark:text-white line-clamp-2 mb-1"><?php echo e($ev->name); ?></h3>
                        <div class="flex items-center justify-between text-xs text-slate-500 dark:text-slate-400 mt-2">
                            <span>
                                <?php if($ev->shoot_date): ?>
                                    <i class="bi bi-calendar3"></i> <?php echo e(\Carbon\Carbon::parse($ev->shoot_date)->format('d M Y')); ?>

                                <?php endif; ?>
                            </span>
                            <?php if(!$ev->is_free && $ev->price_per_photo): ?>
                                <span class="font-bold text-indigo-600 dark:text-indigo-400">
                                    ฿<?php echo e(number_format($ev->price_per_photo, 0)); ?>/ภาพ
                                </span>
                            <?php endif; ?>
                        </div>
                    </div>
    ...
    <section id="reviews" class="max-w-6xl mx-auto px-4 md:px-6 py-12 scroll-mt-20">
        <div class="text-center mb-10">
            ...
            <h2 class="text-3xl md:text-4xl font-extrabold text-slate-900 dark:text-white tracking-tight">
                <?php echo e(number_format($totalReviewCount)); ?> รีวิว
                <?php if($avgRating): ?>
                    <span class="text-amber-500">· <?php echo e(number_format($avgRating, 1)); ?> ★</span>
                <?php endif; ?>
            </h2>
        </div>
                ...
                <article class="bg-white dark:bg-slate-800 rounded-2xl p-5 shadow-sm ring-1 ring-slate-100 dark:ring-slate-700 hover:shadow-md transition">
                    <div class="flex items-center gap-3 mb-3">
                        ...
                        <div class="flex-1 min-w-0">
                            <div class="font-semibold text-slate-900 dark:text-white truncate"><?php echo e($reviewerName); ?></div>
                            <div class="flex items-center gap-1 text-xs text-slate-400 mt-0.5">
                                <?php for($i = 1; $i <= 5; $i++): ?>
                                    <i class="bi bi-star<?php echo e($i <= $review->rating ? '-fill' : ''); ?>"
                                       style="color:<?php echo e($i <= $review->rating ? '#f59e0b' : '#cbd5e1'); ?>;"></i>
                                <?php endfor; ?>
                                <span class="ml-1"><?php echo e($review->created_at->format('d M Y')); ?></span>
                            </div>
                        </div>
                    </div>
                    ...
                    <?php if($review->event): ?>
                        <div class="mt-3 pt-3 border-t border-slate-100 dark:border-slate-700 text-xs text-slate-400 truncate">
                            <i class="bi bi-camera"></i> <?php echo e($review->event->name); ?>
    ...
    <div class="relative rounded-3xl overflow-hidden p-10 md:p-14 text-center shadow-2xl"
        ...
        <div class="relative">
            ...
            <div class="flex flex-wrap items-center justify-center gap-3">
                ...
                <?php elseif($profile->portfolio_url): ?>
                    <a href="<?php echo e($profile->portfolio_url); ?>" target="_blank" rel="noopener"
                       class="inline-flex items-center gap-2 px-7 py-3.5 rounded-xl font-semibold text-base text-white border border-white/30 bg-white/10 backdrop-blur-md hover:bg-white/20 transition">
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\photo-gallery-pgsql\resources\views/public/photographers/show.blade.php ENDPATH**/ ?>