  ...
  <div class="relative z-10 grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8 items-start lg:flex-1 w-full">
    ...
    <div class="lg:col-span-7 xl:col-span-8 flex items-start gap-5 min-w-0">
      <div class="w-14 h-14 rounded-2xl bg-white/15 backdrop-blur-md border border-white/25 flex items-center justify-center text-3xl shrink-0 shadow-xl shadow-black/20">
        <i class="bi <?php echo e($greetIcon); ?>"></i>
      </div>
      ...
      <div class="flex-1 min-w-0">
        ...
        <h1 class="font-bold text-2xl md:text-3xl xl:text-4xl tracking-tight mb-2 leading-tight">
          <?php echo e($greeting); ?>,<br class="hidden sm:block"> <span class="bg-gradient-to-r from-amber-200 via-pink-200 to-violet-200 bg-clip-text text-transparent"><?php echo e($displayName); ?></span>
        </h1>

        
        <div class="flex flex-wrap items-center gap-x-2.5 gap-y-2 text-sm text-white/80">
          <span class="inline-flex items-center gap-1.5">
            <i class="bi bi-calendar3"></i><?php echo e(now()->translatedFormat('l ที่ j F Y')); ?>

          </span>

          <?php if($profile?->photographer_code): ?>
            <span class="inline-flex items-center gap-1.5 text-xs bg-white/10 backdrop-blur border border-white/20 rounded-lg px-2.5 py-1 font-medium">
              <i class="bi bi-person-badge"></i><?php echo e($profile->photographer_code); ?>
          ...
          <span class="inline-flex items-center gap-1.5 text-xs bg-white/10 backdrop-blur border border-white/20 rounded-lg px-2.5 py-1 font-medium"
            ...
            <i class="bi <?php echo e($tierIconClass); ?>"></i>
            <span class="text-white/60 font-bold uppercase tracking-wider">ระดับ</span>
            <span class="font-bold"><?php echo e($tierLabelShort); ?></span>
          </span>

          
          <?php if($photographerPlan ?? null): ?>
            <a href="<?php echo e(route('photographer.subscription.index')); ?>"
               ...
               title="แผนสมัครสมาชิก: <?php echo e($photographerPlan->name); ?> — คลิกเพื่อจัดการแผน">
              <i class="bi <?php echo e($photographerPlan->iconClass()); ?>"
                 style="color: <?php echo e(in_array($photographerPlan->code, ['business','studio','pro']) ? '#fde047' : '#fff'); ?>;"></i>
              <span class="text-white/60 font-bold uppercase tracking-wider">แผน</span>
              <span class="font-bold"><?php echo e($photographerPlan->name); ?></span>
            </a>
          <?php endif; ?>
        </div>
      </div>
    ...
    <div class="lg:col-span-5 xl:col-span-4 lg:justify-self-end w-full lg:max-w-[320px] flex flex-col gap-3">
        ...
        <div class="flex items-start justify-between gap-3">
          <div class="min-w-0 flex-1">
            ...
            <div class="text-2xl md:text-3xl font-black tracking-tight text-white leading-none">
              ฿<?php echo e(number_format($stats['earnings']['this_mo'] ?? 0, 0)); ?>

            </div>
            <?php if(($stats['earnings']['delta_pct'] ?? null) !== null): ?>
              <?php $d = $stats['earnings']['delta_pct']; ?>
              <div class="text-xs mt-2 flex items-center gap-1 <?php echo e($d >= 0 ? 'text-emerald-200' : 'text-rose-200'); ?>">
                <i class="bi <?php echo e($d >= 0 ? 'bi-arrow-up-right' : 'bi-arrow-down-right'); ?>"></i>
                <span class="font-semibold"><?php echo e($d >= 0 ? '+' : ''); ?><?php echo e(number_format($d, 1)); ?>%</span>
      ...
      <div class="grid grid-cols-2 gap-2">
        <a href="<?php echo e(route('photographer.events.create')); ?>"
        ...
        <a href="<?php echo e(route('photographer.events.index')); ?>"
...
<?php $topTierIndex = $proTierEnabled ? 2 : 1; ?>
<?php if($profile && $tierIndex < $topTierIndex): ?>
<div class="pg-card mb-6 p-5 md:p-6">
  <div class="flex items-start justify-between gap-4 flex-wrap mb-5">
    <div class="flex items-center gap-4">
      ...
      <div>
        ...
        <div class="text-base md:text-lg font-black pg-text leading-tight"><?php echo e($tierLabels[$tier] ?? $tier); ?></div>
        <div class="text-xs pg-text-mute mt-1">
          <span class="inline-flex items-center gap-1">
            <i class="bi bi-speedometer2"></i>โปรไฟล์สมบูรณ์ <strong class="pg-text"><?php echo e($completeness); ?>%</strong>
          </span>
        </div>
      </div>
    </div>
    <?php if($nextStep): ?>
    <a href="<?php echo e($nextStep['url']); ?>" class="pg-btn pg-btn--primary">
      <i class="bi <?php echo e($nextStep['icon']); ?>"></i><?php echo e($nextStep['cta']); ?>
  ...
  <div class="pg-stepper mb-4">
    <div class="pg-step <?php echo e($tierIndex >= 0 ? ($tierIndex === 0 ? 'pg-step--current' : 'pg-step--done') : ''); ?>"></div>
    <div class="pg-step <?php echo e($tierIndex >= 1 ? ($tierIndex === 1 ? 'pg-step--current' : 'pg-step--done') : ''); ?>"></div>
    <?php if($proTierEnabled): ?>
    <div class="pg-step <?php echo e($tierIndex >= 2 ? 'pg-step--done' : ''); ?>"></div>
    <?php endif; ?>
  </div>

  <div class="grid <?php echo e($proTierEnabled ? 'grid-cols-3' : 'grid-cols-2'); ?> gap-3 text-xs">
    <div>
      <div class="inline-flex items-center gap-1.5 font-bold text-sm <?php echo e($tierIndex >= 0 ? 'pg-text' : 'pg-text-mute'); ?>">
        <i class="bi <?php echo e($tierIndex > 0 ? 'bi-check-circle-fill text-emerald-500' : 'bi-pencil-square'); ?>"></i>
        Creator
      </div>
      <div class="pg-text-mute text-[11px] mt-0.5">สมัคร + สร้าง event</div>
    </div>
    <div>
      <div class="inline-flex items-center gap-1.5 font-bold text-sm <?php echo e($tierIndex >= 1 ? 'pg-text' : 'pg-text-mute'); ?>">
        <i class="bi <?php echo e($tierIndex > 1 ? 'bi-check-circle-fill text-emerald-500' : ($tierIndex === 1 ? 'bi-shop text-violet-500' : 'bi-lock-fill')); ?>"></i>
        Seller
      </div>
      <div class="pg-text-mute text-[11px] mt-0.5">
        <?php echo e($proTierEnabled ? 'PromptPay + เริ่มขาย' : 'PromptPay + ขายไม่ลิมิต'); ?>

      </div>
    </div>
    <?php if($proTierEnabled): ?>
    <div>
      <div class="inline-flex items-center gap-1.5 font-bold text-sm <?php echo e($tierIndex >= 2 ? 'pg-text' : 'pg-text-mute'); ?>">
        <i class="bi <?php echo e($tierIndex >= 2 ? 'bi-patch-check-fill text-amber-500' : 'bi-lock-fill'); ?>"></i>
        Pro
      </div>
  ...
  <div class="mt-5 p-4 rounded-2xl flex items-start gap-3 border"
    ...
    <div class="w-9 h-9 rounded-xl flex items-center justify-center shrink-0 text-white"
      ...
      <i class="bi <?php echo e($nextStep['icon']); ?>"></i>
    </div>
    <div class="flex-1 min-w-0">
      <div class="font-bold text-sm pg-text"><?php echo e($nextStep['title']); ?></div>
      <div class="text-xs pg-text-soft mt-0.5 leading-relaxed"><?php echo e($nextStep['desc']); ?></div>
    </div>
  </div>
  <?php endif; ?>
</div>
...
<?php if($pending > 0 || $modPending > 0 || $drivePending > 0): ?>
  ...
  <a href="<?php echo e(route('photographer.earnings')); ?>"
    ...
    <div class="flex-1 min-w-0">
      ...
      <div class="text-xs text-amber-800/80 dark:text-amber-200/80 mt-0.5">
        <strong class="text-base font-black">฿<?php echo e(number_format($pending, 0)); ?></strong> กำลังดำเนินการ
      </div>
    </div>
  ...
  <a href="<?php echo e(route('photographer.events.index')); ?>"
    ...
    <div class="flex-1 min-w-0">
      ...
      <div class="text-xs text-rose-800/80 dark:text-rose-200/80 mt-0.5">
        <strong class="text-base font-black"><?php echo e(number_format($modPending)); ?></strong> รูป · โปรดตรวจสอบ
      </div>
    </div>
  ...
  <div class="flex items-center gap-4 p-4 rounded-2xl border
    ...
    <div class="flex-1 min-w-0">
      ...
      <div class="text-xs text-sky-800/80 dark:text-sky-200/80 mt-0.5">
        <strong class="text-base font-black"><?php echo e($drivePending); ?></strong> รายการในคิว
      </div>
    </div>
  </div>
  <?php endif; ?>
...
<?php echo $__env->make('photographer.partials.subscription-widget', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
<?php echo $__env->make('photographer.partials.credits-widget', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
...
<div class="grid grid-cols-2 xl:grid-cols-4 gap-4 mb-6">
  ...
  <div class="pg-card pg-card--hover pg-kpi pg-kpi-events">
    <div class="flex items-start justify-between mb-3">
      ...
      <?php if(($stats['events']['delta_pct'] ?? null) !== null): ?>
        <?php $d = $stats['events']['delta_pct']; ?>
        <span class="pg-chip <?php echo e($d >= 0 ? 'pg-delta-up' : 'pg-delta-down'); ?>">
          <i class="bi <?php echo e($d >= 0 ? 'bi-arrow-up-right' : 'bi-arrow-down-right'); ?>"></i>
          <?php echo e($d >= 0 ? '+' : ''); ?><?php echo e(number_format($d, 1)); ?>%
        </span>
      <?php endif; ?>
    </div>
    ...
    <div class="text-3xl xl:text-4xl font-black pg-text leading-none mb-2 tracking-tight">
      <?php echo e(number_format($stats['events']['total'])); ?>

    </div>
    <div class="text-xs pg-text-soft">
      <span class="font-bold"><?php echo e($stats['events']['active']); ?></span> เปิดขาย ·
      <span class="font-bold"><?php echo e($stats['events']['this_mo']); ?></span> เดือนนี้
    </div>
  </div>
  ...
  <div class="pg-card pg-card--hover pg-kpi pg-kpi-photos">
    <div class="flex items-start justify-between mb-3">
      ...
      <?php if($stats['photos']['pending_moderation'] > 0): ?>
        <span class="pg-chip bg-amber-100 text-amber-700 dark:bg-amber-500/15 dark:text-amber-300">
          <i class="bi bi-exclamation-circle-fill"></i><?php echo e($stats['photos']['pending_moderation']); ?>
    ...
    <div class="text-3xl xl:text-4xl font-black pg-text leading-none mb-2 tracking-tight">
      <?php echo e(number_format($stats['photos']['total'])); ?>

    </div>
    <div class="text-xs pg-text-soft">
      <span class="font-bold"><?php echo e(number_format($stats['photos']['active'])); ?></span> แสดงต่อสาธารณะ
    </div>
  </div>
  ...
  <div class="pg-card pg-card--hover pg-kpi pg-kpi-sales">
    <div class="flex items-start justify-between mb-3">
      ...
      <?php if(($stats['sales']['delta_pct'] ?? null) !== null): ?>
        <?php $d = $stats['sales']['delta_pct']; ?>
        <span class="pg-chip <?php echo e($d >= 0 ? 'pg-delta-up' : 'pg-delta-down'); ?>">
          <i class="bi <?php echo e($d >= 0 ? 'bi-arrow-up-right' : 'bi-arrow-down-right'); ?>"></i>
          <?php echo e($d >= 0 ? '+' : ''); ?><?php echo e(number_format($d, 1)); ?>%
        </span>
    ...
    <div class="text-3xl xl:text-4xl font-black pg-text leading-none mb-2 tracking-tight">
      <?php echo e(number_format($stats['sales']['total'])); ?>

    </div>
    <div class="text-xs pg-text-soft">
      AOV <span class="font-bold">฿<?php echo e(number_format($stats['sales']['aov'], 0)); ?></span> ·
      <span class="font-bold"><?php echo e($stats['sales']['this_mo']); ?></span> เดือนนี้
    </div>
  </div>
  ...
  <div class="pg-card pg-card--hover pg-kpi pg-kpi-earnings">
    ...
    <div class="text-3xl xl:text-4xl font-black pg-text leading-none mb-2 tracking-tight">
      ฿<?php echo e(number_format($stats['earnings']['total'], 0)); ?>

    </div>
    <div class="text-xs pg-text-soft flex items-center gap-2">
      <span class="text-emerald-600 dark:text-emerald-300 font-bold">฿<?php echo e(number_format($stats['earnings']['paid'], 0)); ?></span>รับแล้ว
      <span class="pg-text-mute">·</span>
      <span class="text-amber-600 dark:text-amber-300 font-bold">฿<?php echo e(number_format($stats['earnings']['pending'], 0)); ?></span>รอ
    </div>
  </div>
</div>
...
<div class="pg-card p-4 mb-6">
  <div class="flex flex-wrap gap-2 items-center">
    ...
    <a href="<?php echo e(route('photographer.events.create')); ?>" class="pg-quick"><i class="bi bi-plus-circle"></i>สร้าง Event</a>
    <a href="<?php echo e(route('photographer.events.index')); ?>" class="pg-quick"><i class="bi bi-cloud-upload"></i>อัปโหลดรูป</a>
    <a href="<?php echo e(route('photographer.earnings')); ?>" class="pg-quick"><i class="bi bi-cash"></i>ดูรายได้</a>
    <a href="<?php echo e(route('photographer.analytics')); ?>" class="pg-quick"><i class="bi bi-graph-up-arrow"></i>Analytics</a>
    <a href="<?php echo e(route('photographer.reviews')); ?>" class="pg-quick"><i class="bi bi-star"></i>รีวิว</a>
    <a href="<?php echo e(route('photographer.profile')); ?>" class="pg-quick"><i class="bi bi-person-gear"></i>โปรไฟล์</a>
    <a href="<?php echo e(route('photographer.setup-bank')); ?>" class="pg-quick"><i class="bi bi-bank"></i>บัญชีธนาคาร</a>
  </div>
</div>
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-6">
    ...
    <div class="flex items-start justify-between px-6 py-5 border-b pg-divide flex-wrap gap-2">
      <div>
        ...
        <p class="text-xs pg-text-mute ml-10">
          รวม <span class="font-black pg-text">฿<?php echo e(number_format($stats['revenue']['all'], 0)); ?></span> ตั้งแต่เริ่มต้น
        </p>
      </div>
      <?php if(($stats['revenue']['delta_pct'] ?? null) !== null && $stats['revenue']['delta_pct'] !== 0.0): ?>
        <?php $d = $stats['revenue']['delta_pct']; ?>
        <span class="pg-chip <?php echo e($d >= 0 ? 'pg-delta-up' : 'pg-delta-down'); ?>">
          <i class="bi <?php echo e($d >= 0 ? 'bi-arrow-up-right' : 'bi-arrow-down-right'); ?>"></i>
          เดือนนี้ <?php echo e($d > 0 ? '+' : ''); ?><?php echo e(number_format($d, 1)); ?>%
        </span>
      <?php endif; ?>
    </div>
    <div class="p-5">
      <div class="pg-chart-wrap">
        <svg class="pg-chart-svg" viewBox="0 0 <?php echo e($chartW); ?> <?php echo e($chartH); ?>" preserveAspectRatio="none">
          <defs>
          ...
          <g class="pg-chart-grid">
            <?php for($g = 0; $g <= 4; $g++): ?>
              <?php $gy = $chartPad['top'] + ($plotH * $g / 4); ?>
              <line x1="<?php echo e($chartPad['left']); ?>" y1="<?php echo e($gy); ?>" x2="<?php echo e($chartW - $chartPad['right']); ?>" y2="<?php echo e($gy); ?>"/>
            <?php endfor; ?>
          </g>

          
          <?php if($pathArea): ?><path class="pg-chart-fill" d="<?php echo e($pathArea); ?>"/><?php endif; ?>
          
          <?php if($pathLine): ?><path class="pg-chart-line" d="<?php echo e($pathLine); ?>"/><?php endif; ?>

          
          <?php $__currentLoopData = $chartPoints; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $i => $pt): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <circle class="pg-chart-dot <?php echo e($i === count($chartPoints) - 1 ? 'pg-chart-dot--last' : ''); ?>"
                    cx="<?php echo e(round($pt['x'], 2)); ?>" cy="<?php echo e(round($pt['y'], 2)); ?>"
                    r="<?php echo e($i === count($chartPoints) - 1 ? 6 : 4); ?>"/>
            <text class="pg-chart-label" x="<?php echo e(round($pt['x'], 2)); ?>" y="<?php echo e($chartH - 8); ?>"><?php echo e($pt['label']); ?></text>
            <?php if($pt['value'] > 0 && $i === count($chartPoints) - 1): ?>
              <text class="pg-chart-value" x="<?php echo e(round($pt['x'], 2)); ?>" y="<?php echo e(max(14, round($pt['y'], 2) - 14)); ?>">฿<?php echo e(number_format($pt['value'], 0)); ?></text>
            <?php endif; ?>
  ...
  <div class="pg-card overflow-hidden">
    ...
    <div class="p-2">
      <?php $__empty_1 = true; $__currentLoopData = ($topEvents ?? []); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $i => $ev): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
        <a href="<?php echo e(route('photographer.events.edit', $ev->id)); ?>"
           class="pg-row flex items-center gap-3 px-3 py-2.5 rounded-xl transition">
          <div class="pg-rank <?php echo e($i === 0 ? 'pg-rank--1' : ($i === 1 ? 'pg-rank--2' : ($i === 2 ? 'pg-rank--3' : 'pg-rank--default'))); ?>">
            #<?php echo e($i + 1); ?>

          </div>
          <div class="flex-1 min-w-0">
            <div class="text-sm font-bold pg-text truncate"><?php echo e($ev->name); ?></div>
            <div class="text-xs pg-text-mute">
              <?php echo e($ev->shoot_date?->format('d M Y') ?? 'ไม่ระบุวันที่'); ?>

            </div>
          </div>
          <div class="text-right shrink-0">
            <div class="text-sm font-black text-emerald-600 dark:text-emerald-300">
              ฿<?php echo e(number_format((float) $ev->total_revenue, 0)); ?>
...
<div class="grid grid-cols-1 lg:grid-cols-12 gap-4">
        ...
        <tbody>
          ...
          <tr class="pg-row border-t pg-divide transition">
            <td class="px-5 py-3">
              <div class="flex items-center gap-3">
                <?php if($event->cover_image): ?>
                  <img src="<?php echo e($event->cover_image_url); ?>" alt=""
                ...
                <?php endif; ?>
                <div class="font-bold pg-text truncate max-w-[220px]" title="<?php echo e($event->name); ?>">
            ...
            <td class="px-5 py-3 pg-text-mute whitespace-nowrap">
              <?php echo e($event->shoot_date?->format('d M Y') ?? '-'); ?>
            ...
            <td class="px-5 py-3 whitespace-nowrap">
              ...
              <?php else: ?>
                <span class="font-black pg-text"><?php echo e(number_format((float) $event->price_per_photo, 0)); ?></span>
                <span class="text-xs pg-text-mute">฿</span>
              <?php endif; ?>
            </td>
            <td class="px-5 py-3">
              <span class="font-bold pg-text">
                <?php echo e(number_format((int) ($event->order_count ?? 0))); ?>
            ...
            <td class="px-5 py-3">
              ...
              ?>
              <span class="pg-chip <?php echo e($st[0]); ?>"><?php echo e($st[1]); ?></span>
            </td>
            <td class="px-5 py-3 text-right">
              <a href="<?php echo e(route('photographer.events.edit', $event->id)); ?>"
          ...
          <tr>
            <td colspan="6" class="text-center py-12 pg-empty">
              ...
              <a href="<?php echo e(route('photographer.events.create')); ?>" class="pg-btn pg-btn--primary">
  ...
  <div class="lg:col-span-5 space-y-4">
      ...
      <div class="px-5 py-4 grid grid-cols-2 gap-4 border-b pg-divide">
        <div>
          ...
          <div class="text-xl font-black text-emerald-600 dark:text-emerald-300 tracking-tight">
            ฿<?php echo e(number_format($stats['earnings']['paid'], 0)); ?>
        ...
        <div class="pl-4 border-l pg-divide">
          ...
          <div class="text-xl font-black text-amber-600 dark:text-amber-300 tracking-tight">
            ฿<?php echo e(number_format($stats['earnings']['pending'], 0)); ?>

          </div>
        </div>
      </div>
      <div class="divide-y pg-divide">
        <?php $__empty_1 = true; $__currentLoopData = ($recentEarnings ?? []); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $e): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
          <div class="flex items-center gap-3 px-5 py-3 pg-row transition">
            ...
            <div class="flex-1 min-w-0">
              <div class="text-sm font-bold pg-text truncate">
                <?php echo e($e->event_title ?? ('Order #' . $e->order_id)); ?>

              </div>
              <div class="text-xs pg-text-mute">
                <?php echo e($e->created_at?->diffForHumans()); ?>
            ...
            <div class="text-sm font-black text-emerald-600 dark:text-emerald-300 shrink-0">
              +฿<?php echo e(number_format($e->amount, 0)); ?>
          ...
          <div class="pg-row px-5 py-3 flex items-center gap-3 transition">
            ...
            <div class="flex-1 min-w-0">
              <div class="text-sm pg-text truncate">
                <span class="font-bold"><?php echo e(trim(($o->user?->first_name ?? '') . ' ' . ($o->user?->last_name ?? '')) ?: 'ลูกค้า'); ?></span>
                <span class="pg-text-mute mx-1">·</span>
                <span class="text-xs pg-text-mute truncate"><?php echo e($o->event?->name ?? '—'); ?></span>
              </div>
              <div class="text-[11px] pg-text-mute mt-0.5">
                <?php echo e($o->created_at?->diffForHumans()); ?>

              </div>
            </div>
            <div class="text-sm font-black text-emerald-600 dark:text-emerald-300 shrink-0">
              ฿<?php echo e(number_format((float) $o->total, 0)); ?>
    ...
    <?php if(($recentReviews ?? collect())->isNotEmpty()): ?>
    <div class="pg-card overflow-hidden">
      <div class="px-5 py-4 border-b pg-divide flex items-center justify-between">
        ...
        <a href="<?php echo e(route('photographer.reviews')); ?>"
          ...
          <div class="px-5 py-3">
            <div class="flex items-center gap-2 mb-1">
              <div class="flex gap-0.5 text-amber-500">
                <?php for($i = 1; $i <= 5; $i++): ?>
                  <i class="bi bi-star<?php echo e($i <= (int) ($r->rating ?? 0) ? '-fill' : ''); ?> text-xs"></i>
                <?php endfor; ?>
              </div>
              <span class="text-xs pg-text-mute">
                <?php echo e(trim(($r->user?->first_name ?? '') . ' ' . ($r->user?->last_name ?? '')) ?: 'ลูกค้า'); ?>
            ...
            <?php if(!empty($r->comment)): ?>
              <div class="text-xs pg-text-soft line-clamp-2">
                <?php echo e(\Illuminate\Support\Str::limit($r->comment, 140)); ?>
...
<?php echo $__env->make('layouts.photographer', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\photo-gallery-pgsql\resources\views/photographer/dashboard.blade.php ENDPATH**/ ?>