<?php $__env->startSection('title', 'บทความ & ข่าวสาร'); ?>
  ...
  <div class="relative max-w-6xl mx-auto px-4 py-12 md:py-16">
    ...
    <div class="max-w-2xl mx-auto" x-data>
      <div class="relative group">
        ...
        <div class="relative flex items-center bg-white dark:bg-slate-900/80 backdrop-blur-xl border border-gray-200 dark:border-white/10 rounded-2xl overflow-hidden shadow-lg shadow-indigo-500/10 dark:shadow-black/30 transition-all focus-within:shadow-xl focus-within:shadow-indigo-500/20 focus-within:border-indigo-400/60">
          ...
          <button type="button" id="hero-clear-btn" class="<?php echo e(request('q') ? '' : 'hidden'); ?> pr-4 text-gray-400 dark:text-slate-500 hover:text-slate-700 dark:hover:text-white transition" @click="$el.previousElementSibling.value=''; $dispatch('hero-search', {q:''})">
    ...
    <div class="flex items-center justify-center gap-3 mt-6 flex-wrap">
      <div class="inline-flex items-center gap-2 bg-white/70 dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-full px-4 py-1.5 backdrop-blur-sm">
        ...
        <span class="text-slate-600 dark:text-slate-300/70 text-xs font-medium">ทั้งหมด <strong class="text-slate-800 dark:text-white" id="stat-total"><?php echo e($totalPosts ?? 0); ?></strong> บทความ</span>
      </div>
    </div>
  </div>
...
<div class="py-8" x-data="blogSearch()" x-init="init()" @hero-search.window="query = $event.detail.q; fetchPosts()" @hero-category.window="category = $event.detail.id; updateCategoryChips(); fetchPosts()">
  ...
  <?php if(isset($featuredPosts) && $featuredPosts->count()): ?>
  <section class="mb-12">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <?php $mainFeatured = $featuredPosts->first(); ?>
      <article class="lg:row-span-2 group">
        <a href="<?php echo e(route('blog.show', $mainFeatured->slug)); ?>" class="block bg-white dark:bg-slate-800 border border-gray-100 dark:border-white/10 rounded-2xl overflow-hidden h-full blog-card transition-all duration-300">
          <div class="relative overflow-hidden aspect-[16/9] lg:aspect-auto lg:h-full lg:min-h-[340px]">
            <?php if($mainFeatured->featured_image): ?>
              <img src="<?php echo e(asset('storage/' . $mainFeatured->featured_image)); ?>" alt="<?php echo e($mainFeatured->title); ?>" class="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105">
            <?php else: ?>
            ...
            <div class="absolute bottom-0 left-0 right-0 p-5 sm:p-6">
              <?php if($mainFeatured->category): ?>
                <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold text-white bg-indigo-600/85 backdrop-blur-sm mb-3 shadow-md">
                  <?php echo e($mainFeatured->category->name); ?>

                </span>
              <?php endif; ?>
              <h3 class="text-white font-bold text-lg sm:text-xl lg:text-2xl leading-snug line-clamp-2 mb-2"><?php echo e($mainFeatured->title); ?></h3>
              <?php if($mainFeatured->excerpt): ?>
                <p class="text-white/70 text-sm line-clamp-2 hidden sm:block"><?php echo e($mainFeatured->excerpt); ?></p>
              <?php endif; ?>
              <div class="flex items-center gap-3 text-white/60 text-xs mt-3">
                <?php if($mainFeatured->author): ?>
                  <span><?php echo e($mainFeatured->author->full_name ?? $mainFeatured->author->first_name); ?></span>
                <?php endif; ?>
                <?php if($mainFeatured->published_at): ?>
                  <span><i class="bi bi-calendar3 mr-1"></i><?php echo e($mainFeatured->published_at->locale('th')->translatedFormat('j M Y')); ?></span>
                <?php endif; ?>
                <?php if($mainFeatured->reading_time): ?>
                  <span><i class="bi bi-clock mr-1"></i><?php echo e($mainFeatured->reading_time); ?> นาที</span>
                <?php endif; ?>
              </div>
            </div>
          </div>
      ...
      <?php $__currentLoopData = $featuredPosts->skip(1)->take(2); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $featured): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
      <article class="group">
        <a href="<?php echo e(route('blog.show', $featured->slug)); ?>" class="flex flex-col sm:flex-row bg-white dark:bg-slate-800 border border-gray-100 dark:border-white/10 rounded-2xl overflow-hidden blog-card transition-all duration-300 h-full">
          <div class="relative overflow-hidden sm:w-56 aspect-[16/10] sm:aspect-auto shrink-0">
            <?php if($featured->featured_image): ?>
              <img src="<?php echo e(asset('storage/' . $featured->featured_image)); ?>" alt="<?php echo e($featured->title); ?>" class="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105">
            <?php else: ?>
          ...
          <div class="p-5 flex flex-col justify-center flex-1">
            ...
            <?php endif; ?>
            <h3 class="font-semibold text-sm sm:text-base leading-relaxed line-clamp-2 mb-2 text-slate-800 dark:text-gray-100 group-hover:text-indigo-600 dark:group-hover:text-indigo-300 transition-colors"><?php echo e($featured->title); ?></h3>
            <div class="flex items-center gap-3 text-xs text-gray-500 dark:text-gray-400">
              <?php if($featured->published_at): ?>
                <span><?php echo e($featured->published_at->locale('th')->translatedFormat('j M Y')); ?></span>
              <?php endif; ?>
              <?php if($featured->reading_time): ?>
                <span><i class="bi bi-clock mr-1"></i><?php echo e($featured->reading_time); ?> นาที</span>
              <?php endif; ?>
            </div>
          </div>
        </a>
  ...
  <div class="flex flex-col lg:flex-row gap-8">
      ...
      <div x-show="!loading" x-transition.opacity>
        <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6" id="blog-posts-grid">
          <?php $__currentLoopData = $posts; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $post): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <?php echo $__env->make('public.blog._post-card', ['post' => $post], array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
          <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
        ...
        <div id="blog-pagination" class="my-12">
          <?php echo $__env->make('public.blog._pagination', ['posts' => $posts], array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
        </div>
      </div>
    </div>
    ...
    <div class="hidden lg:block w-80 xl:w-96 shrink-0">
      <div class="sticky top-20">
        <?php echo $__env->make('public.blog._sidebar', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
      </div>
    </div>
  </div>
...
function blogSearch() {
  return {
    ...
    loading: false,
    total: <?php echo e($posts->total()); ?>,
    showing: <?php echo e($posts->count()); ?>,
    ...
    async fetchPosts(page) {
      ...
      try {
        const res = await fetch(`<?php echo e(route("blog.index")); ?>?${params.toString()}`, {
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\photo-gallery-pgsql\resources\views/public/blog/index.blade.php ENDPATH**/ ?>